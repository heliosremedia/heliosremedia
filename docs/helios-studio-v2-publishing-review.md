...
Existing queue action controls now suppress synchronous duplicate clicks. A failed, malformed, timed-out or unexpected acknowledgement disables further actions on that stale job row and asks the administrator to reload the queue. This is client containment, not a change to server retry/approval semantics. A successful response must match the requested transition; no response is treated as provider confirmation. An executable actual-component test with synthetic React hooks and fetch responses covers duplicate requests, network/JSON/access failures, unexpected transitions and confirmed transitions. It is not a browser test of the mutation controls.
