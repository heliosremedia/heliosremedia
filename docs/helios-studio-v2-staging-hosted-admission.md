...
The first isolated build reproduced an import-time R2 credential requirement. The exact staging Preview context now uses a throwing R2 config/client facade, with no SDK construction or provider call. Other contexts retain the existing credential validation and SDK construction. An executable actual-module test covers both paths.
