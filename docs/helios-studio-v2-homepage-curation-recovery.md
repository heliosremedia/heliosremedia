...
An active-only project patch now preserves the omitted title. Final review also reproduced a false conflict when its acknowledgement contained the saved title while the browser held an unsaved title; the browser now merges only fields included in that project mutation. Replaced/removed work-card objects are retained for uncertain-response reconciliation instead of being deleted inline. No cleanup worker or registry lifecycle was introduced. Existing key-prefix/visible-library checks and storage verification remain. Storage verification is a read-only provider check within the bounded transaction; hosted latency/timeout behavior needs rehearsal.
...
Local final combined suite: **785 passed, 0 failed**. Prisma generation without migration, non-incremental TypeScript, scoped lint, fixture bundle and whitespace passed. Final upload merge and active-only project acknowledgement refinements were followed by all 27 component checks; the final successful code CI below verifies those refinements. Local Chromium installation failed on download, so no local browser pass is claimed. Initial CI 35280678899 passed 785 tests, TypeScript and every prior component browser check, but the new standalone fixture failed to render because Next image build-time environment constants were missing. An executable bundled import reproduced `process is not defined`; supplying the build-time constants fixed that import. The final successful CI/Chromium checkpoint below supersedes that failed run.
...
Draft [#311](https://github.com/heliosremedia/heliosremedia/pull/311), `codex/v2-homepage-curation-recovery`, stacked on #310. Code head `3725dc4d95827a3a7aa7cc8db762941098b586a6`, tree `b338cb180f0cd00643ef271ac9307b3b1221b05c`. [CI 35281342479](https://github.com/heliosremedia/heliosremedia/actions/runs/35281342479), job `105403748263`, passed **786/786 tests** and every workflow step. Fresh curation Chromium passed 390px at 2026-09-17 22:24:02 UTC and 1440px at 22:24:10 UTC. Both actual components execute with synthetic fetch/XHR; the checks assert recovery/copy/reload, stale/newer state, ordering, upload completion/remount, removal, no replay, no overflow and no runtime errors. Existing browser checks and isolated anonymous Next.js HTTP also pass. Final documentation head/CI are recorded on the PR and canonical run claim.

Packet 4 is complete for its stated bounded scope and ready for roadmap review. This does not close Phase 1 tenant exit gates, Phase 2 media/job lifecycle gates, hosted restoration or production-parity gates. No subsequent packet is started.
