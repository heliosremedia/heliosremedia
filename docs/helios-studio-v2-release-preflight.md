...
A no-ledger database is not silently converted. The rehearsal proves rejection and uses an independently, actually deployed single-baseline fixture to represent the supported post-establishment state. It does not fake applying historical DML or run resolve. The existing-original-ledger fixture is explicitly synthetic, as in #318; actual deployed Helios history remains unknown.
