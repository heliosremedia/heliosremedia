...
ReferralCampaign now has nullable stored ownership, scoped audience/public reads and legacy execution containment. Lifecycle, snapshot, consent and delivery isolation remain unfinished. ClientPortal now has a nullable ownership expansion, scoped challenge paths and legacy provider containment; its tenant provider configuration remains open. TeamMember, AboutPageContent, EmailCampaign and Inquiry now have nullable ownership expansions; verified backfills, complete flow isolation and contract gates remain open. OperationalIncident and ReferralCronInvocation need platform-versus-tenant visibility rules. AuditEvent now has stored nullable ownership and scoped tenant reads; unclassified history, remaining background writers and platform visibility still need review. CommunicationClient and global marketing preferences/suppression require careful shared-identity/consent handling without weakening existing opt-outs. These are prioritized review targets, not authorization to modify protected provider connections.
