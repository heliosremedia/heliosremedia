...
test("visible version links to the matching code-controlled release", () => {
  assert.equal(STUDIO_VERSION, "V1.8.7");
  assert.equal(STUDIO_VERSION_HREF, "/admin/release-notes/v1-8-7");
  assert.equal(getStudioRelease("v1-8-7")?.version, STUDIO_VERSION);
