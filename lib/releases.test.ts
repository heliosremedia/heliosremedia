...
test("visible version links to the matching code-controlled release", () => {
  assert.equal(STUDIO_VERSION, "V1.8.9.5");
  assert.equal(STUDIO_VERSION_HREF, "/admin/release-notes/v1-8-9-5");
  assert.equal(getStudioRelease("v1-8-9-5")?.version, STUDIO_VERSION);
  assert.equal(getStudioRelease("v1-8-9-5")?.status, "LIVE");
  assert.equal(getStudioRelease("v1-8-9-5")?.releaseDate, "2026-07-28");
