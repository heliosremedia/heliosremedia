...
test("visible version links to the matching code-controlled release", () => {
  assert.equal(STUDIO_VERSION, "V1.8.9.6");
  assert.equal(STUDIO_VERSION_HREF, "/admin/release-notes/v1-8-9-6");
  assert.equal(getStudioRelease("v1-8-9-6")?.version, STUDIO_VERSION);
  assert.equal(getStudioRelease("v1-8-9-6")?.status, "LIVE");
  assert.equal(getStudioRelease("v1-8-9-6")?.releaseDate, "2026-07-28");
