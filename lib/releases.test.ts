...
test("visible version links to the matching code-controlled release", () => {
  ...
  assert.equal(STUDIO_VERSION_HREF, "/admin/release-notes/v1-9-2");
  assert.equal(getStudioRelease("v1-9-2")?.version, STUDIO_VERSION);
  assert.equal(getStudioRelease("v1-9-2")?.status, "DEPLOYING");
  assert.equal(getStudioRelease("v1-9-2")?.releaseDate, null);
