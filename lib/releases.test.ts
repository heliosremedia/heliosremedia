...
test("visible version links to the matching code-controlled release", () => {
  assert.equal(STUDIO_VERSION, "V1.9.0");
  assert.equal(STUDIO_VERSION_HREF, "/admin/release-notes/v1-9-0");
  assert.equal(getStudioRelease("v1-9-0")?.version, STUDIO_VERSION);
  assert.equal(getStudioRelease("v1-9-0")?.status, "DEPLOYING");
  assert.equal(getStudioRelease("v1-9-0")?.releaseDate, null);
