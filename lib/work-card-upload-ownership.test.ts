...
function presignFixture() {
 ...
 const route=load('app/api/admin/homepage-work-cards/presign/route.ts');return{state,run:(revision='current')=>route.POST(new Request('http://localhost/api',{method:'POST',headers:revision?{'x-curation-revision':revision,'x-curation-request':'request1'}:{},body:JSON.stringify({cardId:'c1',kind:'image',fileType:'image/webp',fileSize:10})}))};
}
...
test('mixed-version headerless upload preparation is rejected before registration or signing',async()=>{const f=presignFixture();assert.equal((await f.run('')).status,409);assert.equal(f.state.registered,0);assert.equal(f.state.signed,0);});
