...
test('curation removal retains old object for uncertain acknowledgement recovery', async () => {
 ...
 const modules: Record<string, any> = { '@/lib/work-card-media-server': { resolveWorkCardMedia() { throw new Error('Unexpected media resolution'); } }, 'next/server': { NextResponse: Response }, '@/lib/auth/session': { getAdminSession: async () => ({ role: 'EDITOR', workspaceId: 'a' }) }, '@/lib/content-image-storage': { verifyContentImage() {}, deleteContentImage: async () => { deletions++; } }, '@/lib/homepage-curation-write': { withCurationWrite: async (_actor: any, _scope: any, _request: any, fn: any) => fn(tx, new Date()) } };
 const exports: any = {};
