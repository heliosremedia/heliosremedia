...
test("quick browse is independently configurable, progressive, and tenant isolated", () => {
  ...
  const photos = read("../app/(public)/portfolio/gallery/PhotographyBrowser.tsx");
  const films = read("../app/(public)/portfolio/films/FilmsBrowser.tsx");
  ...
  assert.doesNotMatch(read("../app/(public)/page.tsx"), /PhotographyBrowser|FilmsBrowser|Quick Browse/);
});
...
test("media intent uses durable anchors, focus placement, reduced motion, and fallback", () => {
  ...
  const navigator = read("../app/(public)/portfolio/[slug]/MediaIntentNavigator.tsx");
...
test("gallery routes provide unique metadata and defer media resources", () => {
  assert.match(read("../app/(public)/portfolio/gallery/page.tsx"), /Real Estate Photography Gallery/);
  assert.match(read("../app/(public)/portfolio/films/page.tsx"), /Real Estate Film Gallery/);
  ...
  assert.match(read("../app/(public)/portfolio/films/FilmsBrowser.tsx"), /preload="metadata"/);
  assert.doesNotMatch(read("../app/(public)/portfolio/films/FilmsBrowser.tsx"), /autoPlay/);
});
