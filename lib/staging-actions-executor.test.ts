...
for(const failure of ['', 'PUBLIC', 'ADMIN', 'BROWSER', 'ANONYMOUS_200', 'ANONYMOUS_302', 'ANONYMOUS_403', 'EGRESS'])test('hosted HTTP harness both tenants and fixed redirect diagnostic '+(failure||'success'),async()=>{
 ...
 let contexts=0,closedContexts=0,closedBrowsers=0,aborted=0,continued=0;const widths:number[]=[];
 const response=(status:number,data:unknown)=>({status:()=>status,json:async()=>data,text:async()=>String(data)});
 const browser={close:async()=>{closedBrowsers++;},newContext:async(options:{extraHTTPHeaders:Record<string,string>})=>{assert.deepEqual(options.extraHTTPHeaders,{'x-vercel-skip-toolbar':'1',...(failure?{'x-vercel-protection-bypass':'synthetic-bypass'}:{})});const id=ids[contexts++];let authenticated=false;return {
  close:async()=>{closedContexts++;},addCookies:async()=>{authenticated=true;},route:async(_pattern:string,handler:(route:unknown)=>Promise<unknown>)=>{const origin='https://helios-v2-staging-'+(contexts-1)+'.vercel.app';await handler({request:()=>({url:()=>origin+'/_next/static/test.js'}),abort:async()=>{aborted++;},continue:async()=>{continued++;}});if(failure==='EGRESS')await handler({request:()=>({url:()=> 'https://external.example.test/private?token=private-sentinel',resourceType:()=> 'image'}),abort:async()=>{aborted++;},continue:async()=>{continued++;}});},
 ...
 const run=()=>qualifyHTTP(db,ids.map((workspaceId,i)=>({workspaceId,hostname:'helios-v2-staging-'+i+'.vercel.app'})),'a'.repeat(96),failure?'synthetic-bypass':undefined,{launch:async()=>browser} as unknown as Parameters<typeof qualifyHTTP>[4]);
 if(failure){await assert.rejects(run(),e=>{const d=diagnostic('hosted-http-chromium',e);assert.equal(d.reason,failure==='EGRESS'?'CHECK_HTTP_EGRESS_OTHER_HOST__HTTP_EGRESS_IMAGE__HTTP_BROWSER_EGRESS':'CHECK_HTTP_'+(failure.startsWith('ANONYMOUS_')?'ANONYMOUS':failure)+'_STATUS');assert.equal(d.matched,false);assert.deepEqual(Object.keys(d).sort(),['detailHash','matched','phase','reason']);assert.ok(!JSON.stringify(d).includes('vercel.app'));assert.ok(!JSON.stringify(d).includes('private-sentinel'));return true;});if(failure==='EGRESS'){assert.equal(aborted,1);assert.equal(continued,1);}assert.equal(closedContexts,1);assert.equal(closedBrowsers,1);assert.equal(process.env.AUTH_SECRET,undefined);return;}
