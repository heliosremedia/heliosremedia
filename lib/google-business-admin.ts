...
export async function getGoogleBusinessAdminState(session: AdminSession & { workspaceId: string }) {
  ...
  try {
    const [connection, reviews, importedReviewCount, settings] = await Promise.all([
      ...
      prisma.siteSettings.findFirst({ where: tenantMode ? { workspaceId: session.workspaceId } : { OR: [{ workspaceId: session.workspaceId }, { id: "default" }] }, select: { googleReviewDisplayMode: true } }),
    ]);
    if (tenantMode && !settings) throw new Error("Workspace review display settings are not configured.");
