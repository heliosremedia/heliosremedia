...
test("V1.9.4.5 does not reintroduce global route transitions", () => {
  const layout = read("app/layout.tsx") + read("app/(public)/layout.tsx");
...
test("share control stays compact with a 44 pixel target and visible focus", () => {
  const source = read("app/(public)/portfolio/[slug]/ShareProject.tsx");
...
test("team biographies clamp responsively and expose accessible controls only on overflow", () => {
  const source = read("app/(public)/about/TeamBiography.tsx");
...
test("FAQ, Services, and Blog polish remains data-driven", () => {
  const faq = read("app/(public)/faq/FaqExplorer.tsx");
  ...
  assert.match(read("app/(public)/services/page.tsx"), /className="mt-5 max-w-xl[\s\S]*service\.description/);
  assert.match(read("app/(public)/blog/page.tsx"), /post\.excerpt[\s\S]*mobile-summary mt-3/);
});

test("project metadata uses the selected image through the existing optimizer", () => {
  const page = read("app/(public)/portfolio/[slug]/page.tsx");
