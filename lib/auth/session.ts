...
export async function getAdminSession() {
  ...
  const user = await prisma.adminUser.findUnique({ where: { id: session.userId }, select: { active: true, sessionVersion: true, role: true, email: true, displayName: true, navigationFavorites: true, workspaceId: true } });
  ...
  const navigationFavorites = Array.isArray(user.navigationFavorites)
    ...
    : [];
  return { ...session, role: user.role, email: user.email, displayName: user.displayName, navigationFavorites, workspaceId: user.workspaceId };
}
