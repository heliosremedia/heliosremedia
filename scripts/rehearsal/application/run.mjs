...
try{
 ...
 if(process.argv.includes('--prepare-only')){console.log('PASS generated Prisma driver bundle and safety preparation; database/application not executed');}
  ...
  if(process.argv.includes('--restored-fixture')){
   ...
   for(const [host,id] of [['127.0.0.1','a'],['localhost','b']]){const response=await fetch(targets.candidate+'/',{headers:{host}});assert.equal(response.status,200);const text=await response.text();assert.ok(text.includes('REHEARSAL COMPANY '+id));assert.ok(!text.includes('REHEARSAL COMPANY '+(id==='a'?'b':'a')));}
