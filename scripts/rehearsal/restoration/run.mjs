...
import {HISTORICAL,databaseUrl,requireDatabase,seedHistorical,migrate,backfill,snapshot,digest,DATABASES,catalog,ownershipDistribution} from './core.mjs';
...
const clients=[];const generated=join(process.cwd(),'scripts/rehearsal/restoration/verify.generated.mjs');
...
async function dump(name,path){await command('/usr/lib/postgresql/16/bin/pg_dump',['--dbname',databaseUrl(name),'--format=custom','--no-owner','--no-acl','--file',path]);console.log('BACKUP',name,createHash('sha256').update(await readFile(path)).digest('hex'));}
...
try{
 ...
 console.log('PRE OWNERSHIP',ownershipDistribution(pre));
 const preDump=join(scratch,'pre.dump');await dump('packet10_source',preDump);
 await migrate(source,{negative:true});console.log('EXPANDED OWNERSHIP',ownershipDistribution(await snapshot(source)));await backfill(source,{negative:true});
 ...
 const post=await snapshot(source);const semantic=await snapshot(source,{semantic:true});const postCatalog=await catalog(source);console.log('POST SNAPSHOT',digest(post));console.log('POST OWNERSHIP',ownershipDistribution(post));
 ...
 await build({entryPoints:['scripts/rehearsal/restoration/verify.ts'],outfile:generated,bundle:true,platform:'node',format:'esm',packages:'external'});
 const {verifyRestored}=await import(pathToFileURL(generated));await verifyRestored('packet10_pre_restore');await verifyRestored('packet10_post_restore');
 console.log('PASS Packet10 migration/backup/restore phase; application verification follows');
}finally{await Promise.all(clients.map(c=>c.end()));await control.end();await rm(generated,{force:true});console.log('Synthetic backups retained until isolated environment disposal: '+scratch);}
