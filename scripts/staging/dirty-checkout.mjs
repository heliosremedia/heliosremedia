...
const exact=new Map([
 ...
 ...['next-env.d.ts','tsconfig.tsbuildinfo','prisma.config.ts','next.config.ts','eslint.config.mjs','postcss.config.mjs','vercel.json'].map(name=>[name,order[5]]),
]);
...
export function dirtyCategories(raw,numstat){
 try{
  ...
  for(let i=0;i<fields.length;i+=2){
   ...
   else found.add(exact.get(name)||(/^prisma\/migrations\/(?:[A-Za-z0-9_-]+\/migration\.sql|migration_lock\.toml)$/.test(name)?order[4]:/^app\/generated\/prisma\/[A-Za-z0-9_./-]+$/.test(name)&&!name.split('/').includes('..')?order[5]:other));
  }
