...
import {diagnostic,eventsSummary} from './diagnostics.mjs';
...
async function suppress(){const p=await api(projectPath);assert.equal(p.id,HOSTED.project);assert.equal(p.accountId,HOSTED.team);await api(projectPath,'PATCH',{commandForIgnoringBuildStep:'exit 0'});assert.equal((await api(projectPath)).commandForIgnoringBuildStep,'exit 0');}
async function footprint(){
 ...
 for(const {tablename:t} of tables){assert.match(t,/^[A-Za-z_]+$/);assert.equal(Number((await db.query('SELECT count(*) n FROM "'+t+'"')).rows[0].n),expected.has(t)?2:0,'Unexpected fixture data');}
 for(const id of ['packet16-a','packet16-b']){
  assert.equal((await db.query('SELECT id FROM "Workspace" WHERE id=$1',[id])).rowCount,1);
  assert.equal((await db.query('SELECT u.id FROM "AdminUser" u JOIN "WorkspaceMembership" m ON m."userId"=u.id AND m."workspaceId"=u."workspaceId" WHERE u.id=$1 AND u."workspaceId"=$2 AND u.email=$3 AND u.active=true AND u.role=$4 AND u."passwordHash" IS NULL AND u."sessionVersion"=1 AND m.status=$5 AND m.role=$4',[id+'-owner',id,id+'@example.test','OWNER','ACTIVE'])).rowCount,1);
  assert.equal((await db.query('SELECT id FROM "SiteSettings" WHERE id=$1 AND "workspaceId"=$2',[id+'-settings',id])).rowCount,1);
  assert.equal((await db.query('SELECT h.id FROM "HomepageProject" h JOIN "Project" p ON p.id=h."projectId" WHERE h.id=$1 AND p.id=$2 AND p."workspaceId"=$3',[id+'-placement',id+'-project',id])).rowCount,1);
 }
}
async function unbind(){if(!db)return;await db.query('BEGIN');try{for(const b of bindings){const r=await db.query('UPDATE "WorkspaceDomain" SET hostname=$1 WHERE "workspaceId"=$2 AND hostname=$3',[b.workspaceId+'.example.test',b.workspaceId,b.hostname]);if(r.rowCount===0)assert.equal((await db.query('SELECT id FROM "WorkspaceDomain" WHERE "workspaceId"=$1 AND hostname=$2',[b.workspaceId,b.workspaceId+'.example.test'])).rowCount,1);else assert.equal(r.rowCount,1);}await db.query('COMMIT');bindings=[];}catch(e){await db.query('ROLLBACK');throw e;}}
...
try{
 invocation(env);assert.ok(env.STAGING_VERCEL_TOKEN);assert.equal(execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),env.GITHUB_SHA);
 assert.equal(execFileSync('git',['-C','.candidate','rev-parse','HEAD'],{encoding:'utf8'}).trim(),CANDIDATE);
 execFileSync('git',['-C','.candidate','diff','--quiet','HEAD','--']);
 if(process.argv[2]==='--restore-suppression'){const journal=JSON.parse(await readFile('staging-evidence/restoration-needed.json','utf8'));assert.equal(journal.run,env.GITHUB_RUN_ID);assert.equal(journal.executor,env.GITHUB_SHA);await suppress();console.log('STAGING_SUPPRESSION_RESTORED');}
 else{assert.equal(process.argv.length,2);await execute({
  preflight:async()=>{
   const gh=env.STAGING_GITHUB_READ_TOKEN||env.GITHUB_TOKEN,g='https://api.github.com/repos/heliosremedia/heliosremedia';
   protection(await jsonGet(g+'/environments/'+ENVIRONMENT,gh),await jsonGet(g+'/environments/'+ENVIRONMENT+'/deployment-branch-policies',gh));
   const run=await jsonGet(g+'/actions/runs/'+RELEASE_RUN,gh);assert.equal(run.head_sha,CANDIDATE);assert.equal(run.head_branch,HOSTED.branch);assert.equal(run.status,'completed');assert.equal(run.conclusion,'success');assert.equal(run.path,'.github/workflows/v2-regression.yml');
   project(await api(projectPath),await api(projectPath+'/domains'));environmentInventory((await api(projectPath+'/env')).envs);
   const n='https://console.neon.tech/api/v2/projects/'+TARGET.project,t=env.STAGING_NEON_API_KEY;
   const [p,b,e,d]=await Promise.all([jsonGet(n,t),jsonGet(n+'/branches/'+TARGET.branch,t),jsonGet(n+'/endpoints',t),jsonGet(n+'/branches/'+TARGET.branch+'/databases',t)]);
   db=new pg.Client(targetConnection({project:p.project,branch:b.branch,endpoints:e.endpoints,databases:d.databases},env.STAGING_DIRECT_URL));await db.connect();
   assert.equal((await db.query('SELECT pg_try_advisory_lock(1200320) locked')).rows[0].locked,true);before=await databaseState(db,manifest);await footprint();
   for(const id of ['packet16-a','packet16-b'])assert.equal((await db.query('SELECT id FROM "WorkspaceDomain" WHERE "workspaceId"=$1 AND hostname=$2 AND status=$3 AND purpose=$4',[id,id+'.example.test','ACTIVE','PUBLIC_SITE'])).rowCount,1);
