...
export async function qualifyHTTP(db,bindings,secret,bypass,browserType=chromium){
 ...
 try{for(const {workspaceId:id,hostname} of bindings){
  ...
  try{
   const publicResponse=await context.request.get(origin+'/',{maxRedirects:0,timeout:30000});assert.equal(publicResponse.status(),200);
   ...
   const admin=await context.request.get(origin+'/admin/homepage',{maxRedirects:0,timeout:30000});assert.equal(admin.status(),200);
   ...
   for(const width of [390,1440]){const page=await context.newPage();await page.setViewportSize({width,height:900});const errors=[];page.on('pageerror',()=>errors.push('pageerror'));
    const response=await page.goto(origin+'/admin/homepage',{waitUntil:'networkidle',timeout:60000});assert.equal(response?.status(),200);await page.getByRole('heading',{name:'Homepage curation',exact:true}).waitFor();assert.equal(errors.length,0);await page.close();}
