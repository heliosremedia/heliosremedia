...
-- each record's ownership. Accepted kinds: BlogPost, BlogSeries, NewsletterSeries, NewsletterImageAsset.
...
BEGIN;
LOCK TABLE "BlogPost", "BlogSeries", "NewsletterSeries", "NewsletterImageAsset" IN SHARE ROW EXCLUSIVE MODE;
DO $$
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE w.id IS NULL OR m.kind NOT IN ('BlogPost','BlogSeries','NewsletterSeries','NewsletterImageAsset')
  ...
  IF EXISTS (
    ...
    LEFT JOIN (
      ...
      UNION ALL SELECT 'NewsletterImageAsset', id, "workspaceId" FROM "NewsletterImageAsset"
...
UPDATE "NewsletterImageAsset" r SET "workspaceId"=m."workspaceId"
FROM pg_temp."ContentOwnershipMapping" m WHERE m.kind='NewsletterImageAsset' AND m.id=r.id AND r."workspaceId" IS NULL;
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM "BlogPost" WHERE "workspaceId" IS NULL)
    ...
    OR EXISTS (SELECT 1 FROM "NewsletterImageAsset" WHERE "workspaceId" IS NULL)
  ...
  IF EXISTS (SELECT 1 FROM "BlogPost" p JOIN "NewsletterImageAsset" a ON a."storageKey"=p."featuredImageStorageKey" WHERE p."workspaceId"<>a."workspaceId")
  THEN RAISE EXCEPTION 'Blog generated image ownership mismatch'; END IF;
END $$;
COMMIT;
