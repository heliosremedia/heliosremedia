...
if (process.argv.includes('--bundle-only')) {
  console.log('PASS: curation editor fixture bundled; no browser verification performed');
} else {
  const files = {
    '/': ['text/html', '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Synthetic curation editor</title><link rel="stylesheet" href="/style.css"></head><body style="background:#090909;color:white;padding:16px;max-width:1400px;margin:auto"><main id="root"></main><script src="/fixture.js"></script></body></html>'],
  ...
  try {
    ...
    const { checkCurationEditor } = await import('./curation-editor-check.mjs');
    await checkCurationEditor(`http://127.0.0.1:${server.address().port}`);
