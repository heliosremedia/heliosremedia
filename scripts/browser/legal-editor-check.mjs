...
export async function checkLegalEditor(base) {
  ...
  try {
    ...
    assert.match(await article(0).textContent(), /Saved: draft/);
    ...
    assert.match(await article(0).textContent(), /Saved: published/);
    ...
    for (const mode of ['lost-ack', 'conflict', 'forbidden', 'non-json', 'foreign', 'wrong-type', 'stale', 'invalid-revision', 'wrong-publication', 'legacy']) {
      await setup(mode);
    ...
    assert.equal(await page.evaluate(() => window.legalFixture.calls.length), 1);
    await setup('slow-json'); await start(); await finish();
    await page.waitForFunction(() => !!window.legalFixture.jsonPending);
    await page.clock.fastForward(30001); await page.getByRole('alert').waitFor();
    await page.evaluate(() => window.legalFixture.remount());
    await page.waitForFunction(() => !document.querySelector('[role="alert"]'));
    await article(0).getByLabel('Page title').fill('New editor draft');
    await article(1).getByLabel('Page title').fill('New terms draft');
    await page.evaluate(() => window.legalFixture.jsonPending()); await page.clock.runFor(10);
    assert.equal(await article(0).getByLabel('Page title').inputValue(), 'New editor draft', 'late old JSON cannot overwrite a newer editor');
    assert.equal(await article(1).getByLabel('Page title').inputValue(), 'New terms draft');
    assert.equal(await page.evaluate(() => window.legalFixture.calls.length), 1, 'new editor never retries the old publication');
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), true);
    assert.deepEqual(errors, []); assert.deepEqual(unexpected, []);
    console.log(`PASS: real legal editor Chromium ${viewport.width}px: duplicate/frozen saves, confirmed publication, creation, ten held responses, copied draft recovery, fetch/JSON timeouts and late response after remount; synthetic fetch only`);
