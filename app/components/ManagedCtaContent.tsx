...
export default function ManagedCtaContent({ cta }: { cta: PublicCta }) {
  return <section className="relative overflow-hidden border-y border-white/[0.08] bg-[#0d0d0d]">
    ...
    <div className="container-shell relative flex flex-col gap-10 py-24 sm:py-32 lg:flex-row lg:items-end lg:justify-between">
      <div>
        ...
        <h2 className={`${cta.eyebrow ? "mt-6" : ""} max-w-4xl font-display text-[clamp(3rem,6.5vw,6rem)] font-light leading-[0.98] tracking-[-0.052em] text-white`}>{cta.headline}</h2>
        {cta.body && <p className="mt-9 max-w-xl text-sm leading-7 text-white/38">{cta.body}</p>}
      </div>
