...
export default async function BookingPage() {
  ...
  const online = destination.kind === "handoff" && settings.bookingHandoffEnabled;
  const company = settings.businessName || "our studio"; const provider = settings.bookingProviderName || "our secure booking partner";
  return <main className="min-h-screen bg-[#090909] text-white"><Navbar variant="solid"/><section className="container-shell pb-24 pt-16 sm:pt-24"><PublicPageHeading eyebrow={settings.bookingEyebrow||(online?"Secure booking handoff":paused?"Booking paused":"Booking status")} headline={online?`Continue to schedule with ${company}.`:settings.bookingHeadline||(paused?"Bookings are currently paused.":"Online booking is temporarily unavailable.")} summary={online?`You are leaving ${company} for ${provider}. If that service is unavailable, contact the studio directly and we will help.`:settings.bookingExplanation||`Please contact ${company} directly and we will help coordinate your project.`} metadata={!online&&settings.bookingEstimatedRestoreAt?<>Estimated restoration: {new Intl.DateTimeFormat("en-US",{dateStyle:"medium",timeStyle:"short"}).format(new Date(settings.bookingEstimatedRestoreAt))}</>:undefined} actions={<>{online&&<a href={destination.href} rel="noopener noreferrer" className="admin-btn-primary">{settings.bookingPrimaryLabel||"Continue to booking"}</a>}{settings.bookingPhoneVisible&&phone&&<a href={`tel:${settings.bookingContactPhone||settings.phoneE164}`} className={online?"admin-btn-secondary":"admin-btn-primary"}>{settings.bookingCallLabel||`Call ${phone}`}</a>}{settings.bookingEmailVisible&&email&&<a href={`mailto:${email}`} className="admin-btn-secondary">{settings.bookingEmailLabel||"Email us"}</a>}</>}/>{!online&&settings.bookingRequestEnabled&&<BookingRequestForm/>}</section><Footer/></main>;
}
