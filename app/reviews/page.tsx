  ...
  return <main className="min-h-screen bg-[#080808] text-white">
    ...
    <section className="relative overflow-hidden border-b border-white/[0.07]">
      ...
      <div className="container-shell relative py-14 sm:py-16 lg:py-20">
        ...
        <p className="mt-8 max-w-2xl text-base leading-8 text-white/48 sm:text-lg">Selected Google reviews from the agents and teams who trust Helios to bring their listings to life.</p>
      </div>
    </section>

    <section className="container-shell py-16 sm:py-20 lg:py-24" aria-label="Google reviews">
      <p aria-label={`${averageRating.toFixed(1)} out of 5 stars from ${aggregate._count._all} Google reviews`} className="mb-7 text-[0.6rem] font-semibold uppercase tracking-[0.16em] text-white/35 sm:mb-8"><span aria-hidden="true" className="mr-2 text-[var(--helios-orange)]">{"★".repeat(roundedRating)}<span className="text-white/15">{"★".repeat(5 - roundedRating)}</span></span>{averageRating.toFixed(1)} on Google · {aggregate._count._all} reviews</p>
