  ...
  return <>
    <div className="grid gap-7 xl:grid-cols-[0.85fr_1.15fr]">
      ...
      <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-[#111]">
        ...
        {users.map(user => <article key={user.id} className="grid gap-4 border-b border-white/[0.06] p-5 last:border-0 lg:grid-cols-[minmax(0,1fr)_12rem_9rem_auto] lg:items-center"><div className="min-w-0"><p className="truncate text-sm text-white/70">{user.displayName}{user.id === currentUserId ? " · You" : ""}</p><p className="mt-1 truncate text-xs text-white/30">{user.email} · {user.lastLoginAt ? `Last signed in ${new Date(user.lastLoginAt).toLocaleDateString()}` : "Never signed in"}</p></div><label className="text-[0.62rem] text-white/25">Professional title<input defaultValue={user.title || ""} maxLength={120} onBlur={event => { if (event.currentTarget.value.trim() !== (user.title || "")) void updateTitle(user, event.currentTarget.value.trim()); }} className="mt-1 w-full rounded-xl border border-white/10 bg-black/25 px-3 py-2.5 text-xs text-white"/></label><select aria-label={`Permission role for ${user.displayName}`} disabled={busy || (user.role === "OWNER" && currentRole !== "OWNER")} value={user.role} onChange={event => update(user, { role: event.target.value as AdminRole })} className="rounded-xl border border-white/10 bg-black/25 px-3 py-2.5 text-xs text-white">{roleOptions.map(item => <option key={item}>{item}</option>)}</select><div className="flex flex-wrap gap-2"><button type="button" disabled={busy || (user.role === "OWNER" && currentRole !== "OWNER")} onClick={() => { setResetUser(user); setPassword(""); setMessage(null); }} className="admin-btn-secondary">Reset password</button><button type="button" disabled={busy || user.id === currentUserId || (user.role === "OWNER" && currentRole !== "OWNER")} onClick={() => update(user, { active: !user.active })} className={user.active ? "admin-btn-danger" : "admin-btn-secondary"}>{user.active ? "Deactivate" : "Reactivate"}</button></div></article>)}
      </section>
    </div>
