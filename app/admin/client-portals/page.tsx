...
export default async function ClientPortalsPage() {
  ...
  const portals = await prisma.clientPortal.findMany({ where: await getContentOwnershipScope(session.workspaceId), orderBy: [{ displayOrder: "asc" }, { name: "asc" }], select: { id: true, name: true, slug: true, description: true, provider: true, hdphGroupId: true, loginUrl: true, registrationUrl: true, bookingUrl: true, registrationEnabled: true, isDefault: true, active: true, displayOrder: true } });
