...
export default async function AdminCtasPage() {
  ...
  const ctas = await prisma.callToAction.findMany({ where: await getContentOwnershipScope(session.workspaceId), orderBy: { createdAt: "asc" }, include: { placements: { select: { slot: true } } } });
