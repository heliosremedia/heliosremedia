  ...
  return (
    <>
      ...
      <section className="rounded-2xl border border-white/[0.08] bg-[#111] p-5 sm:p-7">
        <div className="flex flex-col gap-5 border-b border-white/[0.08] pb-6 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow text-[var(--helios-orange)]">Partner presentation</p><h2 className="mt-2 text-2xl font-light text-white">Organization logos</h2><p className="mt-2 text-sm text-white/30">Published logos flow through the homepage rail in this order.</p></div><button type="button" onClick={openCreate} className="self-start admin-btn-primary sm:self-auto">Add logo</button></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {logos.map((logo, index) => <article key={logo.id} className={`overflow-hidden rounded-2xl border bg-white/[0.02] ${logo.published ? "border-white/[0.09]" : "border-white/[0.06] opacity-65"}`}>
            ...
            <div className="p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-sm text-white/75">{logo.organizationName}</h3><p className="mt-1 truncate text-xs text-white/25">{logo.websiteUrl || "No destination link"}</p></div><span className={`rounded-full border px-2 py-1 text-[0.47rem] uppercase tracking-[0.12em] ${logo.published ? "border-emerald-300/15 text-emerald-200/55" : "border-white/10 text-white/25"}`}>{logo.published ? "Published" : "Draft"}</span></div>
              <div className="mt-4 flex items-center gap-2 border-t border-white/[0.06] pt-3"><button type="button" aria-label={`Move ${logo.organizationName} up`} disabled={index === 0 || busy !== null} onClick={() => move(index, -1)} className="admin-btn-icon-sm text-lg">↑</button><button type="button" aria-label={`Move ${logo.organizationName} down`} disabled={index === logos.length - 1 || busy !== null} onClick={() => move(index, 1)} className="admin-btn-icon-sm text-lg">↓</button><button type="button" disabled={busy !== null} onClick={() => toggle(logo)} className="ml-auto admin-btn-link">{logo.published ? "Unpublish" : "Publish"}</button><button type="button" onClick={() => openEdit(logo)} className="admin-btn-link">Edit</button><button type="button" disabled={busy !== null} onClick={() => remove(logo)} className="admin-btn-link-destructive">Delete</button></div>
            </div>
          </article>)}
