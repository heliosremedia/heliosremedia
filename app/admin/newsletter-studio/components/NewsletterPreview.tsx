...
export default function NewsletterPreview({ edition, mode, selectedId }: { edition: NewsletterEdition; mode: "desktop" | "mobile"; selectedId?: string | null }) {
  return <div className={`mx-auto overflow-hidden rounded-xl border border-black/10 bg-[#f2eee5] shadow-2xl transition-[max-width] ${mode === "mobile" ? "max-w-[390px]" : "max-w-[680px]"}`}>
    ...
    <div className="bg-[#f2eee5] text-[#23201d]">{edition.blocks.map(block => <section key={block.id} data-newsletter-preview-block={block.id} className={`${selectedId === block.id ? "newsletter-preview-highlight " : ""}${block.type === "DIVIDER" || block.type === "SPACER" ? "px-8 py-4" : "px-7 py-7 sm:px-10"} ${block.alignment === "center" ? "text-center" : "text-left"}`}>
