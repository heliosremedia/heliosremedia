...
export default function NewsletterPreview({ edition, mode }: { edition: NewsletterEdition; mode: "desktop" | "mobile" }) {
  return <div className={`mx-auto overflow-hidden rounded-xl border border-black/10 bg-[#f2eee5] shadow-2xl transition-[max-width] ${mode === "mobile" ? "max-w-[390px]" : "max-w-[680px]"}`}>
    ...
    <div className="bg-[#f2eee5] text-[#23201d]">{edition.blocks.map(block => <section key={block.id} className={`${block.type === "DIVIDER" || block.type === "SPACER" ? "px-8 py-4" : "px-7 py-7 sm:px-10"} ${block.alignment === "center" ? "text-center" : "text-left"}`}>
      {block.type === "DIVIDER" ? <hr className="border-[#23201d]/15" /> : block.type === "SPACER" ? <div className="h-6" /> : <>{block.imageUrl && <Image unoptimized width={640} height={360} src={block.imageUrl} alt={block.altText || ""} className="mb-6 aspect-[16/9] w-full object-cover" />}{block.eyebrow && <p className="text-[9px] font-bold uppercase tracking-[.2em] text-[#c75f28]">{block.eyebrow}</p>}{block.heading && <h2 className="mt-2 font-serif text-[clamp(1.55rem,5vw,2.4rem)] font-normal leading-tight">{block.heading}</h2>}{block.body && <p className="mt-4 whitespace-pre-wrap text-[15px] leading-7 text-[#39332e]">{block.body}</p>}{block.link && block.buttonLabel && <a href={block.link} tabIndex={-1} className="mt-6 inline-block bg-[#171717] px-5 py-3 text-[10px] font-bold uppercase tracking-[.13em] text-white">{block.buttonLabel}</a>}</>}</section>)}</div>
