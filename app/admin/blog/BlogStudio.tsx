...
export default function BlogStudio({initialPosts,images,defaultAuthor,series,summary}:{initialPosts:BlogEditorPost[];images:BlogImageOption[];defaultAuthor:string;series:BlogSeriesEditor[];summary:ReactNode}) {
  ...
  const [imageGeneratorOpen,setImageGeneratorOpen]=useState(false); const [imagePrompt,setImagePrompt]=useState(""); const [imageAltText,setImageAltText]=useState("");
  const [imageGenerating,setImageGenerating]=useState(false); const [generatedImage,setGeneratedImage]=useState<{assetId:string;storageKey:string;url:string;altText:string;attribution:string}|null>(null);
  ...
  async function restorePrevious(){if(!activeId)return;setBusy(true);setMessage("Finding previous version…");try{const response=await fetch(`/api/admin/blog/revisions?postId=${activeId}`);const result=await response.json();const revision=result.revisions?.[0];if(!response.ok||!revision)throw new Error("No previous version is available.");if(!confirm(`Restore the version from ${new Date(revision.createdAt).toLocaleString()}?`))return;const restored=await fetch("/api/admin/blog/revisions",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({postId:activeId,revisionId:revision.id})});const data=await restored.json();if(!restored.ok||!data.success)throw new Error(data.error||"Restore failed.");const post={...data.post,sourceLinks:Array.isArray(data.post.sourceLinks)?data.post.sourceLinks:[]};setDraft(post);setPosts(current=>current.map(item=>item.id===activeId?post:item));setMessage("Previous version restored ✓");}catch(error){setMessage(error instanceof Error?error.message:"Restore failed.");}finally{setBusy(false);}}
  async function ai(action:string){
    if(action==="seo"&&(draft.seoTitle||draft.seoDescription)&&!confirm("Replace the current SEO content with the generated improvements?"))return;
    if(action==="social"&&draft.socialCaption&&!confirm("Replace the current social caption?"))return;
    const textarea=contentRef.current;const selectionStart=textarea?.selectionStart||0;const selectionEnd=textarea?.selectionEnd||0;
    const selected=draft.content.slice(selectionStart,selectionEnd);
    const source=action==="draft"||action==="outline"?brief:action==="rewriteSelection"?selected:draft.content;if(!source.trim())return setMessage(action==="rewriteSelection"?"Select the passage you want AI to rewrite.":"Add a brief or article first.");
  ...
  function openImageGenerator(){
    const context={title:draft.title,excerpt:draft.excerpt,content:draft.content,category:draft.category,seoTitle:draft.seoTitle};
    const prepared=prepareBlogImagePrompt(context);
    if(!prepared)return setMessage("Add a title, excerpt, or article content before generating a featured image.");
    setImagePrompt(prepared);setImageAltText(draft.featuredImageAlt||suggestBlogImageAltText(context));setGeneratedImage(null);setImageGeneratorOpen(true);setMessage(null);
  }
  async function generateFeaturedImage(){
    setImageGenerating(true);setMessage(null);
    try{const response=await fetch("/api/admin/blog/images/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({prompt:imagePrompt,altText:imageAltText})});const data=await response.json();if(!response.ok||!data.success)throw new Error(data.error||"The image could not be generated.");setGeneratedImage(data.image);}
    catch(error){setMessage(error instanceof Error?error.message:"The image could not be generated.");}
    finally{setImageGenerating(false);}
  }
  function approveGeneratedImage(){
    if(!generatedImage)return;
    if(imageUrl&&!confirm("Replace the current featured image with this AI-generated image?"))return;
    setDraft(current=>({...current,featuredMediaId:null,featuredImageStorageKey:generatedImage.storageKey,featuredImageUrl:generatedImage.url,featuredImageAlt:imageAltText}));
    setImageGeneratorOpen(false);setGeneratedImage(null);setMessage("AI featured image selected. Save the article to keep this change.");
  }
  function surprise(){
  ...
  >
    ...
    <div className="grid gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
      ...
      <main className="admin-form-scope space-y-6 overflow-hidden rounded-2xl border border-white/[0.08] bg-[#111] p-5 sm:p-7">
        <section className="rounded-xl border border-[var(--helios-orange)]/20 bg-[var(--helios-orange)]/[0.04] p-5"><label className="text-xs text-white/45">AI writing brief<textarea rows={3} value={brief} onChange={e=>setBrief(e.target.value)} placeholder="Example: Explain why twilight photography helps luxury listings stand out in Northern Colorado…" className={input}/></label><div className="mt-3 flex flex-wrap items-center gap-2"><button disabled={busy} onClick={()=>ai("draft")} className="admin-btn-primary">Generate draft</button><button disabled={busy} onClick={()=>ai("outline")} className="admin-btn-secondary">Generate outline</button>{message==="AI is writing…"&&<span role="status" className="ml-1 text-sm text-emerald-300">AI is writing…</span>}{draft.content&&<><button disabled={busy} onClick={()=>ai("rewriteSelection")} className="admin-btn-secondary">Rewrite selection</button><button disabled={busy} onClick={()=>ai("improve")} className="admin-btn-secondary">Adjust tone</button><button disabled={busy} onClick={()=>ai("shorten")} className="admin-btn-secondary">Shorten</button><button disabled={busy} onClick={()=>ai("expand")} className="admin-btn-secondary">Expand</button><button disabled={busy} onClick={()=>ai("headings")} className="admin-btn-secondary">Suggest headings</button><button disabled={busy} onClick={()=>ai("seo")} className="admin-btn-secondary">Improve SEO</button><button disabled={busy} onClick={()=>ai("social")} className="admin-btn-secondary">Social captions</button></>}</div><p className="mt-3 text-xs leading-5 text-white/30">AI always writes into a draft. Selection tools change only the highlighted passage. Review facts, links, and claims before publishing.</p></section>
        ...
        <div className="flex flex-wrap items-center gap-3"><button disabled={busy} onClick={save} className="admin-btn-primary">{busy?"Working…":"Save article"}</button>{activeId&&<Link href={`/blog/${draft.slug}`} target="_blank" className="admin-btn-secondary">Preview ↗</Link>}{activeId&&<button disabled={busy} onClick={restorePrevious} className="admin-btn-secondary">Restore previous version</button>}{activeId&&<button disabled={busy} onClick={remove} className="admin-btn-destructive">Delete</button>}</div>
      </main>
    </div>
