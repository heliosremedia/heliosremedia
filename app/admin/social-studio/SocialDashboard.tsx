...
export default function SocialDashboard({ campaigns, projects, blogs, newsletters, summary }: {
  ...
  const [cadenceStart] = useState(() => Date.now());
  const [message, setMessage] = useState("");
  const [creationMode, setCreationMode] = useState<"manual" | "ai">("manual");
  const [draft, setDraft] = useState({ internalName: "", sourceType: "PROJECT", sourceRecordId: "", purpose: "", topic: "", objective: objectives[0], targetAudience: "", primaryMessage: "", callToAction: "", destinationLink: "", tone: "Use Social Studio Voice", mediaPreference: "Recommend the strongest verified media and format", internalAiInstructions: "", platforms: ["INSTAGRAM", "FACEBOOK"] });
  ...
  async function create() {
    ...
    try {
      ...
      if (creationMode === "ai") {
        setMessage("AI is preparing your campaign…");
        const generated = await fetch("/api/admin/social/ai", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ campaignId: data.campaign.id, action: "create-platform-variants", requestId: crypto.randomUUID(), tone: draft.tone }) });
        const generatedData = await generated.json();
        if (!generated.ok || !generatedData.success) throw new Error(generatedData.error || "The campaign was saved, but AI generation failed. Your brief is preserved.");
      }
  ...
  return <>
    <section className="flex flex-col gap-5 border-b border-white/[0.08] pb-7 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Link href="/admin/social-studio/analytics" className="admin-btn-secondary">Social analytics</Link><Link href="/admin/social-studio/queue" className="admin-btn-secondary">Publishing queue</Link><Link href="/admin/social-studio/calendar" className="admin-btn-secondary">Content calendar</Link><Link href="/admin/social-studio/settings" className="admin-btn-secondary">Voice & connections</Link><button onClick={() => setOpen(true)} className="admin-btn-primary">New campaign</button></div>
    </section>
    ...
    {open && <div role="dialog" aria-modal="true" aria-labelledby="new-social-title" className="fixed inset-0 z-[100] overflow-y-auto bg-black/85 p-4 backdrop-blur">
      ...
      <div className="mt-6 grid grid-cols-2 gap-2 rounded-xl border border-white/10 bg-black/20 p-1" aria-label="Campaign creation method"><button type="button" onClick={() => setCreationMode("manual")} className={creationMode === "manual" ? "admin-btn-primary" : "admin-btn-secondary"}>Create manually</button><button type="button" onClick={() => setCreationMode("ai")} className={creationMode === "ai" ? "admin-btn-primary" : "admin-btn-secondary"}>Create with AI</button></div>
      ...
      <label className="text-xs text-white/40 sm:col-span-2">{creationMode === "ai" ? "Large campaign brief" : "Purpose or creative brief"}<textarea rows={creationMode === "ai" ? 7 : 4} className={input} value={draft.purpose} onChange={(e) => setDraft({ ...draft, purpose: e.target.value })}/></label>{creationMode === "ai" && <><label className="text-xs text-white/40">Topic<input className={input} value={draft.topic} onChange={(e) => setDraft({ ...draft, topic: e.target.value, primaryMessage: e.target.value })}/></label><label className="text-xs text-white/40">Tone<input className={input} value={draft.tone} onChange={(e) => setDraft({ ...draft, tone: e.target.value })}/></label><label className="text-xs text-white/40 sm:col-span-2">Media preference<input className={input} value={draft.mediaPreference} onChange={(e) => setDraft({ ...draft, mediaPreference: e.target.value, purpose: `${draft.purpose}\nMedia preference: ${e.target.value}`.trim() })}/></label></>}<label className="text-xs text-white/40">Objective<select className={input} value={draft.objective} onChange={(e) => setDraft({ ...draft, objective: e.target.value })}>{objectives.map((item) => <option key={item}>{item}</option>)}</select></label><label className="text-xs text-white/40">Audience<input className={input} value={draft.targetAudience} onChange={(e) => setDraft({ ...draft, targetAudience: e.target.value })}/></label>
      ...
      {message && <p role={saving ? "status" : "alert"} aria-live="polite" className="mt-5 text-sm text-white/60">{message}</p>}<div className="mt-7 flex justify-end gap-3"><button disabled={saving} onClick={() => setOpen(false)} className="admin-btn-secondary">Cancel</button><button disabled={saving || !draft.internalName || !draft.platforms.length || (creationMode === "ai" && !draft.purpose.trim())} onClick={create} className="admin-btn-primary">{saving ? creationMode === "ai" ? "AI is preparing your campaign…" : "Creating…" : creationMode === "ai" ? "Create AI drafts" : "Create campaign"}</button></div></div></div>}
  </>;
}
