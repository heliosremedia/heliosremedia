...
const actionMessages: Record<string, string> = {
  ...
  "send-now": "Approved post added to the protected publishing queue.",
...
export default function SocialCampaignEditor({ initialCampaign, library, connections }: { initialCampaign: Campaign; library: Media[]; connections:Connection[] }) {
  ...
  async function action(actionName: string, extra: Record<string, unknown> = {}) {
    ...
    try {
      ...
      if (["approve","schedule","send-now","publish","archive","archive-campaign","submit-review","request-changes","set-media"].includes(actionName)) {
  ...
  return <div className="space-y-7 pb-10">
    ...
    <div className="grid gap-6 xl:grid-cols-[13rem_minmax(0,1fr)_22rem]">
      ...
      <section className="min-w-0 rounded-2xl border border-white/[.08] bg-white/[.02] p-5 sm:p-6">
        ...
        {variant.status==="APPROVED"&&connections.some(item=>item.platform===variant.platform)&&<div className="mt-5 rounded-xl border border-[var(--helios-orange)]/20 bg-[var(--helios-orange)]/[.04] p-4"><p className="text-sm text-white/65">Send now</p><p className="mt-2 text-xs leading-5 text-white/35">Uses the same immutable snapshot, queue, and duplicate protection as a scheduled post.</p><div className="mt-3 flex flex-wrap gap-2"><select aria-label="Immediate publishing destination" className={input} value={connectionId} onChange={event=>setConnectionId(event.target.value)}><option value="">Choose destination…</option>{connections.filter(item=>item.platform===variant.platform).map(item=><option key={item.id} value={item.id}>{item.label}</option>)}</select><button disabled={!connectionId||Boolean(busy)} onClick={()=>window.confirm("Publish this approved revision now to the selected destination?")&&action("send-now",{connectionId})} className="admin-btn-primary">Confirm send now</button></div></div>}
