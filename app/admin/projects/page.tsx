...
export default async function ProjectsPage({ searchParams }: ProjectsPageProps) {
  ...
  const projects = await prisma.project.findMany({
    where: {
      ...
      ...(search ? { OR: [
        ...
        { details: { is: { propertyAddress: { contains: search, mode: "insensitive" as const } } } },
  ...
  return <div className="space-y-7">
    ...
    <section className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-4 sm:p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
      <form className="flex w-full max-w-xl items-center rounded-xl border border-white/[0.08] bg-black/20 px-4 focus-within:border-white/20"><span className="text-white/30">⌕</span><input type="search" name="search" defaultValue={search} placeholder="Search titles, locations, or property addresses" className="h-11 w-full bg-transparent px-3 text-sm text-white outline-none placeholder:text-white/25" />{activeStatus !== "ALL" ? <input type="hidden" name="status" value={activeStatus} /> : null}<button type="submit" className="text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-white/40 hover:text-white">Search</button></form>
