...
function ProjectRow({ project, sortable }: { project: AdminProjectListItem; sortable: boolean }) {
...
export default function ProjectListManager({ initialProjects, hasFilters }: { initialProjects: AdminProjectListItem[]; hasFilters: boolean }) {
  ...
  async function onDragEnd(event: DragEndEvent) {
    ...
    try {
      const response = await fetch("/api/admin/projects/order", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ projectIds: next.map((item) => item.id) }) });
