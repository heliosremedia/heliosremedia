...
export default function FeaturedProjectsManager({ initialFeatured, candidates, overLimitCount }: { initialFeatured: ProjectOption[]; candidates: ProjectOption[]; overLimitCount: number }) {
  ...
  function onDragStart(event: DragStartEvent) { setActiveId(String(event.active.id)); setOverId(String(event.active.id)); }
  function onDragOver(event: DragOverEvent) { setOverId(event.over ? String(event.over.id) : null); }
