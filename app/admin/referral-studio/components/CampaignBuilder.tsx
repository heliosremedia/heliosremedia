...
const field = "admin-input mt-2 w-full";
...
export default function CampaignBuilder({ adminEmail, initialClientId }: { adminEmail: string; initialClientId: string }) {
  ...
  return <form ref={formRef} onSubmit={submit} className="space-y-7">
    ...
    <Section eyebrow="02 · Audience" title="Choose who may be invited">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2"><div><p className={label}>Client groups</p><div className="mt-2 max-h-72 space-y-2 overflow-auto rounded-xl border border-white/[0.08] p-3">{groups.map(group => <label key={group.id} className="flex cursor-pointer items-center gap-3 rounded-lg p-2 text-sm text-white/55 hover:bg-white/[0.03]"><input type="checkbox" name="groupIds" value={group.id} className="accent-[var(--helios-orange)]" /><span className="min-w-0 flex-1 truncate">{group.name}</span><span className="text-xs text-white/25">{group.count}</span></label>)}{!groups.length && <p className="p-2 text-xs text-white/30">No groups available.</p>}</div></div><div><label className={label}>Individual clients<input value={clientSearch} onChange={event => setClientSearch(event.target.value)} placeholder="Search by name or email" className={field} /></label><div className="mt-2 max-h-72 space-y-2 overflow-auto rounded-xl border border-white/[0.08] p-3">{clients.filter(client => `${client.displayName} ${client.email}`.toLowerCase().includes(clientSearch.toLowerCase())).slice(0, 100).map(client => <label key={client.id} className="flex cursor-pointer items-center gap-3 rounded-lg p-2 text-sm text-white/55 hover:bg-white/[0.03]"><input type="checkbox" name="clientIds" value={client.id} defaultChecked={client.id === initialClientId} disabled={!client.emailSubscribed || client.emailStatus !== "VALID"} className="accent-[var(--helios-orange)]" /><span className="min-w-0 flex-1"><span className="block truncate">{client.displayName}</span><span className="block truncate text-xs text-white/25">{client.email}</span></span>{(!client.emailSubscribed || client.emailStatus !== "VALID") && <span className="text-[0.5rem] uppercase tracking-[.1em] text-amber-100/60">Ineligible</span>}</label>)}</div></div></div>
    ...
    <Section eyebrow="07 · Follow-up" title="Conservative follow-up">
      <div className="flex items-center gap-3"><input id="followUpEnabled" name="followUpEnabled" type="checkbox" className="h-4 w-4 accent-[var(--helios-orange)]" /><label htmlFor="followUpEnabled" className="text-sm text-white/60">Enable approved follow-up for advocates who have not submitted a referral</label></div>
