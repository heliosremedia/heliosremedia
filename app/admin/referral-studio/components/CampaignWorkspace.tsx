...
type Campaign = {
  ...
  operationalState: string; operationalLabel: string; invitationSentCount: number;
  deliveryScheduledAt: string | null; deliveryTimezone: string; scheduleConfirmedAt: string | null;
  nextScheduledAt: string | null; nextAction: string; lastWorkerActivityAt: string | null; lastProviderActivityAt: string | null;
...
export default function CampaignWorkspace({ campaignId, adminEmail }: { campaignId: string; adminEmail: string }) {
  ...
  async function scheduleCampaign(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); setBusy("schedule"); setMessage(null);
    try {
      const response = await fetch(`/api/admin/referrals/campaigns/${campaignId}`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "schedule", firstSendAt: new Date(firstSendAt).toISOString(),
          timezone: campaign?.deliveryTimezone || "America/Denver", confirmation: "SCHEDULE",
        }),
      });
      const result = await response.json();
      if (!response.ok || !result.success) throw new Error(result.error || "The schedule could not be confirmed.");
      setMessage(result.message); setScheduleOpen(false); await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "The schedule could not be confirmed."); }
    finally { setBusy(null); }
  }
  ...
  return <div className="space-y-6">
    <header className="flex flex-col gap-5 border-b border-white/[0.08] pb-7 xl:flex-row xl:items-end xl:justify-between">
      <div><Link href="/admin/referral-studio" className="text-xs text-white/35 transition hover:text-white">← Referral Studio</Link><div className="mt-4 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-light text-white sm:text-4xl">{campaign.internalName}</h1><span className={`rounded-full border px-2.5 py-1 text-[0.54rem] uppercase tracking-[.14em] ${tone(campaign.operationalState)}`}>{campaign.operationalLabel}</span></div><p className="mt-3 max-w-3xl text-sm leading-6 text-white/40">{campaign.purpose}</p></div>
      <div className="flex flex-wrap gap-2">
        ...
        {campaign.status === "APPROVED" && campaign.preparedAdvocateCount === 0 && <button disabled={!!busy} onClick={() => action("launch")} className="admin-btn-primary">{busy === "launch" ? "Preparing Campaign…" : "Prepare campaign"}</button>}
        {campaign.status === "APPROVED" && campaign.preparedAdvocateCount > 0 && !campaign.scheduleConfirmedAt && <button disabled={!!busy} onClick={() => setScheduleOpen(true)} className="admin-btn-primary">Review &amp; Schedule</button>}
    ...
    <section aria-labelledby="campaign-status-title" className="rounded-2xl border border-white/[0.09] bg-gradient-to-br from-white/[0.04] to-transparent p-5 sm:p-7">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="text-[0.56rem] font-semibold uppercase tracking-[.18em] text-[var(--helios-orange)]">Operational status</p><h2 id="campaign-status-title" className="mt-3 text-2xl font-light text-white">{campaign.operationalLabel} · {campaign.expectedAdvocateCount ?? campaign.audienceEstimate.eligible.length} advocates · {campaign.invitationSentCount} invitations sent</h2><p className="mt-2 text-sm text-white/45">Next action: {campaign.nextAction}</p></div><div className="text-left lg:text-right"><p className="text-sm text-white/65">Next send: {campaign.nextScheduledAt ? new Date(campaign.nextScheduledAt).toLocaleString() : "Not scheduled"}</p><p className="mt-1 text-xs text-white/30">{campaign.deliveryTimezone || "America/Denver"}</p></div></div>
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Metric label="Approved advocates" value={campaign.expectedAdvocateCount ?? 0} /><Metric label="Prepared advocates" value={campaign.preparedAdvocateCount} /><Metric label="Follow-ups scheduled" value={campaign.communicationCounts.SCHEDULED ? Math.max(0, campaign.communicationCounts.SCHEDULED - (campaign.expectedAdvocateCount ?? 0)) : 0} /><Metric label="Failed / suppressed" value={(campaign.communicationCounts.FAILED ?? 0) + campaign.audienceEstimate.excluded.length} /></div>
      <div className="mt-5 grid gap-3 text-xs text-white/40 sm:grid-cols-2 lg:grid-cols-4"><p>Delivery mode: approval required</p><p>{campaign.sequence.steps}-message sequence · {campaign.sequence.estimatedMessages} estimated messages</p><p>Worker: {campaign.lastWorkerActivityAt ? new Date(campaign.lastWorkerActivityAt).toLocaleString() : "No activity recorded"}</p><p>Provider: {campaign.lastProviderActivityAt ? new Date(campaign.lastProviderActivityAt).toLocaleString() : "No activity recorded"}</p></div>
    </section>
    {campaign.status === "LAUNCHING" && <section aria-live="polite" className={`rounded-xl border px-5 py-5 ${campaign.launchFailedAt || campaign.stalled ? "border-red-300/15 bg-red-300/[0.04]" : "border-[#e7ddc8]/15 bg-[#e7ddc8]/[0.04]"}`}>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="text-sm text-white/70">{campaign.stalled ? "Campaign preparation is stalled." : campaign.launchFailedAt ? "Campaign preparation stopped before completion." : "Preparing campaign…"}</p><p className="mt-1 text-xs leading-5 text-white/35">{campaign.stalled ? "Automatic recovery is disabled. Review the verified counts below and choose an explicit recovery action." : campaign.launchFailedAt ? (campaign.lastLaunchError || "Retry is safe and will not create duplicates.") : `Preparing referral advocates and scheduled invitations securely. You may leave this page. ${campaign.preparedAdvocateCount} of ${campaign.expectedAdvocateCount ?? 0} advocates prepared.`}</p></div>
    ...
    {scheduleOpen && <div role="presentation" className="fixed inset-0 z-[100] grid place-items-center overflow-y-auto bg-black/75 p-4 backdrop-blur-sm" onMouseDown={event => { if (event.currentTarget === event.target) setScheduleOpen(false); }}><form role="dialog" aria-modal="true" aria-labelledby="schedule-title" onSubmit={scheduleCampaign} className="my-6 w-full max-w-3xl rounded-2xl border border-white/10 bg-[#141412] p-6 shadow-2xl sm:p-8"><p className="text-[0.56rem] uppercase tracking-[.18em] text-[var(--helios-orange)]">Final delivery review</p><h2 id="schedule-title" className="mt-3 text-3xl font-light text-white">Review &amp; Schedule</h2><p className="mt-3 text-sm leading-6 text-white/40">Nothing is sent when you open this review. Confirming creates the future schedule shown below.</p><div className="mt-6 grid gap-4 sm:grid-cols-2"><Metric label="Campaign" value={campaign.internalName} /><Metric label="Eligible audience" value={campaign.expectedAdvocateCount ?? 0} /><Metric label="Excluded / suppressed" value={campaign.audienceEstimate.excluded.length} /><Metric label="Estimated total messages" value={campaign.sequence.estimatedMessages} /></div><div className="mt-6 grid gap-5 lg:grid-cols-2"><div className="rounded-xl border border-white/[0.07] p-4"><Data label="Sender" value={[campaign.senderName, campaign.senderEmail].filter(Boolean).join(" · ")} /><Data label="Reply-to" value={campaign.replyTo || adminEmail} /><Data label="Subject" value={campaign.invitationSubject} /><Data label="Preview text" value={campaign.invitationPreviewText} /></div><div className="rounded-xl border border-white/[0.07] p-4"><Data label="Sequence" value={`${campaign.sequence.steps} messages: initial invitation${campaign.sequence.followUps ? ` + ${campaign.sequence.followUps} follow-ups` : ""}`} /><Data label="Delivery mode" value="Administrator approval required" /><Data label="Duplicate protection" value="One invitation and one instance of each sequence step per approved advocate" /><Data label="Time zone" value={campaign.deliveryTimezone} /></div></div><div className="mt-6 rounded-xl border border-white/[0.07] bg-white/[0.02] p-5"><p className="text-[0.54rem] uppercase tracking-[.15em] text-white/30">Initial invitation preview</p><p className="mt-3 text-sm font-medium text-white/70">{campaign.invitationSubject}</p><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-white/45">{campaign.invitationBody}</p></div><label className="mt-6 block text-[0.56rem] uppercase tracking-[.15em] text-white/35">Proposed first send<input autoFocus required type="datetime-local" value={firstSendAt} onChange={event => setFirstSendAt(event.target.value)} className="admin-input mt-2 w-full" /></label><p className="mt-4 text-xs leading-5 text-amber-100/65">By confirming, you authorize creation of the future delivery schedule for {campaign.expectedAdvocateCount ?? 0} advocates. This does not send immediately.</p><div className="mt-7 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><button type="button" onClick={() => setScheduleOpen(false)} className="admin-btn-link">Cancel and return</button><button disabled={busy === "schedule" || !firstSendAt} className="admin-btn-primary">{busy === "schedule" ? "Confirming…" : "Confirm future schedule"}</button></div></form></div>}
