...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  ...
  const [dashboard, monitor, user] = await Promise.all([
    getDashboardData(days), getPublicMonitorSummary(),
  ...
  const statuses = [
    ...
    ["Client sync", relationships.data.lastSync ? `Confirmed ${fmt(relationships.data.lastSync, true)}` : "Not confirmed", "/admin/clients"],
