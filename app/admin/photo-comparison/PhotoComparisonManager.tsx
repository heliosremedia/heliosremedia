...
export default function PhotoComparisonManager({ initialPage }: { initialPage: PageValue }) {
  ...
  function addPair() {
    setPage((current) => ({ ...current, pairs: [...current.pairs, { id: `new-${crypto.randomUUID()}`, label: `Comparison ${current.pairs.length + 1}`, alt: "Describe the room and property", caption: "Drag to compare the overall visual direction.", active: true, position: current.pairs.length, standardImageStorageKey: null, standardImageUrl: "/photo-finishes/standard-kitchen.jpg", editorialImageStorageKey: null, editorialImageUrl: "/photo-finishes/editorial-kitchen.jpg" }] }));
  }
