...
export default function PhotoComparisonManager({ initialPage }: { initialPage: PageValue }) {
  ...
  function addPair() {
    setPage((current) => ({ ...current, pairs: [...current.pairs, { id: `new-${crypto.randomUUID()}`, label: `Comparison ${current.pairs.length + 1}`, editorialStyle: null, alt: "Describe the room and property", caption: "Drag to compare the overall visual direction.", active: true, position: current.pairs.length, standardImageStorageKey: null, standardImageUrl: "", editorialImageStorageKey: null, editorialImageUrl: "" }] }));
  }

  async function save() {
    setBusy(true); setMessage("");
    try { const data = await jsonRequest("/api/admin/photo-comparison", { method: "PATCH", body: JSON.stringify(page) }); setPage(data.page); setMessage("Photo comparison saved."); }
