...
export default async function HomepageCurationPage() {
  ...
  const [placements, projects, workCards, services, films, settings, user] = await Promise.all([
    prisma.homepageProject.findMany({ where: { project: { workspaceId: session.workspaceId } }, orderBy: [{ displayOrder: "asc" }, { createdAt: "asc" }], select: { id: true, projectId: true, titleOverride: true, displayOrder: true, active: true, project: { select: { title: true, slug: true, status: true, locationLabel: true, heroMedia: { select: { storageKey: true, altText: true } } } } } }),
