...
export default async function HomepageCurationPage() {
  const [placements, projects, workCards, services, films] = await Promise.all([
  ...
  return <div className="space-y-7"><section className="border-b border-white/[0.08] pb-7"><p className="eyebrow text-[var(--helios-orange)]">Public presentation</p><h1 className="mt-3 text-3xl font-light tracking-[-0.03em] text-white sm:text-4xl">Homepage curation</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-white/40">Choose an optional Featured Project, then control the five service cards that always follow it in Our Work.</p></section><section><div className="mb-5"><p className="eyebrow text-[var(--helios-orange)]">Optional lead feature</p><h2 className="mt-2 text-2xl font-light text-white">Featured Project hero</h2><p className="mt-3 text-sm text-white/38">Select one published project to lead the section. The five service cards remain visible beneath it.</p></div><HomepageProjectManager initialPlacements={serialized} projects={projects as ProjectOption[]} /></section><HomepageWorkCardManager initialCards={workCards as WorkCard[]} services={services as ServiceOption[]} films={filmOptions} /></div>;
}
