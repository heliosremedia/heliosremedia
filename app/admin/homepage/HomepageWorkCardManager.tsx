...
export default function HomepageWorkCardManager({ initialCards, services, films }: { initialCards: WorkCard[]; services: ServiceOption[]; films: FilmOption[] }) {
  ...
  return <section className="space-y-5 rounded-2xl border border-white/[0.08] bg-[#111] p-5 sm:p-7">
    <div><p className="eyebrow text-[var(--helios-orange)]">Our Work service cards</p><h2 className="mt-2 text-2xl font-light text-white">Five-card service collection</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-white/38">These cards always appear beneath the optional Featured Project. Choose the service, image, destination, and whether a card displays a static image or a muted film.</p></div>
    <div className="flex flex-col gap-3 rounded-xl border border-white/[0.07] bg-black/20 p-4 sm:flex-row">
      ...
      <button type="button" onClick={() => void add()} disabled={!selectedService || busy !== null || cards.length >= 5} className="rounded-full bg-[var(--helios-orange)] px-6 py-3 text-[0.54rem] font-semibold uppercase tracking-[0.14em] text-black disabled:opacity-35">Add service card</button>
    </div>
      ...
      return <article key={card.id} className={`overflow-hidden rounded-2xl border border-white/[0.08] bg-black/20 ${card.active ? "" : "opacity-60"}`}>
        <div className="grid lg:grid-cols-[16rem_minmax(0,1fr)]">
          ...
          <div className="space-y-5 p-5">
            <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-lg text-white/85">{card.service.name}</h3><p className="mt-1 text-xs text-white/25">/portfolio?service={card.service.slug}</p></div><div className="flex gap-2"><button disabled={index === 0 || busy !== null} onClick={() => void move(index, -1)} className="px-2 text-white/40 disabled:opacity-20">↑</button><button disabled={index === cards.length - 1 || busy !== null} onClick={() => void move(index, 1)} className="px-2 text-white/40 disabled:opacity-20">↓</button></div></div>
            <div className="grid gap-4 sm:grid-cols-2"><label className="text-[0.52rem] uppercase tracking-[0.14em] text-white/35">Card title<input value={card.titleOverride || ""} onChange={(event) => updateLocal({ titleOverride: event.target.value })} placeholder={card.service.name} className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm normal-case tracking-normal text-white" /></label><label className="text-[0.52rem] uppercase tracking-[0.14em] text-white/35">Destination<input value={card.destinationOverride || ""} onChange={(event) => updateLocal({ destinationOverride: event.target.value })} className="mt-2 w-full rounded-xl border border-white/10 bg-black/30 px-4 py-3 text-sm normal-case tracking-normal text-white" /></label></div>
            ...
            <div className="grid gap-4 sm:grid-cols-2"><label className="text-[0.52rem] uppercase tracking-[0.14em] text-white/35">Card media<select value={card.mediaMode} onChange={(event) => updateLocal({ mediaMode: event.target.value as WorkCard["mediaMode"] })} className="mt-2 min-h-11 w-full rounded-xl border border-white/10 bg-[#111] px-4 text-sm normal-case tracking-normal text-white"><option value="IMAGE">Static image</option><option value="LIBRARY_VIDEO">Published film</option><option value="UPLOADED_VIDEO">Uploaded looping preview</option></select></label>{card.mediaMode === "LIBRARY_VIDEO" ? <label className="text-[0.52rem] uppercase tracking-[0.14em] text-white/35">Featured film<select value={card.featuredMediaId || ""} onChange={(event) => updateLocal({ featuredMediaId: event.target.value || null })} className="mt-2 min-h-11 w-full rounded-xl border border-white/10 bg-[#111] px-4 text-sm normal-case tracking-normal text-white"><option value="">Choose a published film</option>{films.map((film) => <option key={film.id} value={film.id}>{film.label} · {film.provider}</option>)}</select></label> : <div className="flex items-end"><label className="inline-flex min-h-11 cursor-pointer items-center rounded-full border border-white/15 px-5 text-[0.52rem] font-semibold uppercase tracking-[0.13em] text-white/60">{card.mediaMode === "UPLOADED_VIDEO" && card.videoUrl ? "Replace preview" : "Upload looping preview"}<input type="file" accept="video/mp4,video/webm" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(card, "video", file); event.target.value = ""; }} /></label></div>}</div>
            <div className="flex flex-wrap items-center gap-3 border-t border-white/[0.07] pt-4"><label className="inline-flex min-h-11 cursor-pointer items-center rounded-full border border-white/15 px-5 text-[0.52rem] font-semibold uppercase tracking-[0.13em] text-white/60">{card.imageUrl ? "Replace image" : "Upload image"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void upload(card, "image", file); event.target.value = ""; }} /></label><button type="button" onClick={() => updateLocal({ active: !card.active })} className="text-[0.52rem] uppercase tracking-[0.13em] text-white/45">{card.active ? "Hide card" : "Show card"}</button><button type="button" onClick={() => void remove(card)} className="text-[0.52rem] uppercase tracking-[0.13em] text-red-200/45">Remove</button><button type="button" disabled={busy !== null} onClick={() => void save(card)} className="ml-auto rounded-full bg-[var(--helios-orange)] px-6 py-3 text-[0.52rem] font-semibold uppercase tracking-[0.13em] text-black disabled:opacity-35">{busy === card.id ? "Saving…" : "Save card"}</button></div>
          </div>
        </div>
      </article>;
    })}</div>
