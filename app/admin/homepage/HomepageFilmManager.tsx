  ...
  return <section className="rounded-2xl border border-white/[0.08] bg-[#111] p-5 sm:p-7">
    <div className="grid gap-7 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-white/[0.08] bg-black/25 p-5"><p className="text-[0.55rem] font-semibold uppercase tracking-[0.16em] text-white/45">Featured film</p><p className="mt-3 text-sm text-white/65">MP4 or WebM · 16:9 · under 500 MB</p><p className="mt-2 truncate text-xs text-white/25">{settings.featuredFilmVideoUrl || "No film uploaded"}</p><label className={`mt-5 inline-flex cursor-pointer rounded-full bg-[var(--helios-orange)] px-5 py-3 text-[0.52rem] font-semibold uppercase tracking-[0.14em] text-white ${locked ? "pointer-events-none opacity-40" : ""}`}>{settings.featuredFilmVideoUrl ? "Replace film" : "Upload film"}<input type="file" accept="video/mp4,video/webm" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia("video", file); event.target.value = ""; }} /></label></div>
        <div className="rounded-2xl border border-white/[0.08] bg-black/25 p-5"><p className="text-[0.55rem] font-semibold uppercase tracking-[0.16em] text-white/45">Poster fallback</p><p className="mt-3 text-sm text-white/65">JPG, PNG, WebP, or AVIF · 1920×1080</p><p className="mt-2 truncate text-xs text-white/25">{settings.featuredFilmPosterUrl || "Uses the current card image"}</p><label className={`mt-5 inline-flex cursor-pointer rounded-full border border-white/15 px-5 py-3 text-[0.52rem] font-semibold uppercase tracking-[0.14em] text-white/70 ${locked ? "pointer-events-none opacity-40" : ""}`}>{settings.featuredFilmPosterUrl ? "Replace poster" : "Upload poster"}<input type="file" accept="image/jpeg,image/png,image/webp,image/avif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadMedia("poster", file); event.target.value = ""; }} /></label></div>
      </div>
    </div>
    ...
    <div className="mt-5 flex items-center justify-between gap-4"><p role="status" className="text-xs text-white/30">{message || "The static Cinematic Films image remains the fallback."}</p><button type="button" disabled={locked} onClick={() => void save()} className="rounded-full bg-[var(--helios-orange)] px-6 py-3 text-[0.54rem] font-semibold uppercase tracking-[0.14em] text-black disabled:opacity-40">{busy === "save" ? "Saving…" : "Save feature"}</button></div>
  </section>;
}
