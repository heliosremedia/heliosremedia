          ...
          <div className="mx-auto my-5 w-full max-w-5xl rounded-2xl border border-white/[0.1] bg-[#151515] p-6 shadow-2xl sm:p-8">
            ...
            <div className="mt-7 grid gap-7 lg:grid-cols-[18rem_minmax(0,1fr)]">
              ...
              <div className="grid content-start gap-5 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2"><Field label="Testimonial"><textarea required maxLength={TESTIMONIAL_CHARACTER_LIMIT} rows={6} value={draft.testimonial} onChange={(event) => setDraft({ ...draft, testimonial: event.target.value })} className="mt-2 w-full resize-y rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-sm normal-case leading-7 tracking-normal text-white outline-none focus:border-[var(--helios-orange)]" /></Field><div className="mt-2 flex items-start justify-between gap-4"><p className="max-w-xl text-[0.62rem] normal-case leading-5 tracking-normal text-white/28">Keep the story concise so the agent name, brokerage, and rating always remain visible on the homepage card.</p><p className={`shrink-0 text-[0.55rem] ${draft.testimonial.length >= TESTIMONIAL_CHARACTER_LIMIT ? "text-[var(--helios-orange)]" : "text-white/25"}`}>{draft.testimonial.length} / {TESTIMONIAL_CHARACTER_LIMIT}</p></div></div>
