  ...
  return <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_20rem]">
    ...
    <aside className="space-y-6 xl:self-start">
      <section className="relative rounded-2xl border border-white/[0.08] bg-[#111] p-5">
        <div className="flex items-center justify-between gap-3"><p className="email-studio-preview-label text-xs uppercase tracking-[0.16em] text-white/30">Live preview</p><div className="flex items-center gap-1"><button type="button" onClick={() => setPersonalizedPreview((value) => !value)} className="email-studio-preview-control rounded-md px-2 py-2 text-xs text-[#e7ddc8]/60 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]">{personalizedPreview ? "Personalized" : "Template"}</button><button type="button" onClick={() => setPreviewExpanded(true)} className="email-studio-preview-control rounded-md px-2 py-2 text-xs text-[#e7ddc8]/75 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]" aria-label="Expand live email preview">Expand</button></div></div>
      ...
      <section className="email-studio-history relative z-10 rounded-2xl border border-white/[0.08] bg-[#111] p-5"><p className="email-studio-history-heading text-[0.64rem] uppercase tracking-[0.16em] text-[var(--helios-orange)]">Campaign history</p><p className="email-studio-history-helper mt-1 text-[0.64rem] leading-5 text-white/25">Resume drafts or inspect delivered email snapshots.</p><div className="mt-3 divide-y divide-white/[0.06]">{history.map((campaign) => <article key={campaign.id} className="py-3 first:pt-0"><div className="flex items-start justify-between gap-3"><p className="email-studio-history-subject line-clamp-2 text-[0.74rem] leading-[1.15rem] text-white/65">{campaign.subject}</p><span className="email-studio-history-status shrink-0 text-[8px] uppercase tracking-[0.12em] text-white/30">{campaign.status}</span></div><p className="email-studio-history-meta mt-1 text-[0.62rem] leading-[1.05rem] text-white/25">{campaignHistorySummary(campaign)} · {new Date(campaign.createdAt).toLocaleDateString()}</p>{campaign.status === "DRAFT" ? <div className="relative z-20 mt-2 flex flex-wrap items-center gap-1"><button type="button" disabled={busy !== null} onClick={() => resumeDraft(campaign)} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] font-medium text-[#e7ddc8]/75 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Resume Draft</button><button type="button" disabled={busy !== null} onClick={() => deleteDraft(campaign)} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] text-red-200/55 transition hover:bg-red-400/[0.08] hover:text-red-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 disabled:opacity-40">{busy === `delete:${campaign.id}` ? "Deleting…" : "Delete Draft"}</button></div> : <button type="button" ref={campaign.id === viewingCampaign?.id ? historyTriggerRef : undefined} aria-label={`View campaign: ${campaign.subject}`} onClick={(event)=>{historyTriggerRef.current=event.currentTarget;setViewingCampaign(campaign);}} className="email-studio-history-action mt-2 rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]">View</button>}{campaign.status === "SCHEDULED" && <div className="relative z-20 mt-2 flex flex-wrap gap-1"><button type="button" disabled={busy !== null} onClick={() => openSchedule(campaign)} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Reschedule</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "send-now")} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Send Now</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "edit")} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] text-white/40 transition hover:bg-white/[0.07] hover:text-white/75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Edit</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "cancel")} className="email-studio-history-action rounded-md px-2 py-2 text-[0.46rem] text-red-200/55 transition hover:bg-red-400/[0.08] hover:text-red-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 disabled:opacity-40">Cancel</button></div>}</article>)}{!history.length && <p className="py-6 text-[0.72rem] text-white/30">No campaigns yet.</p>}</div></section>
    </aside>
    {previewExpanded && <div role="dialog" aria-modal="true" aria-labelledby="expanded-preview-title" className="fixed inset-0 z-[120] overflow-y-auto bg-black/90 p-3 backdrop-blur-sm sm:p-8"><div className="mx-auto max-w-5xl rounded-2xl border border-white/10 bg-[#111] p-4 shadow-2xl sm:p-7"><div className="sticky top-0 z-10 -mx-1 flex items-center justify-between gap-4 rounded-xl border border-white/[0.08] bg-[#111]/95 px-4 py-3 backdrop-blur"><div><p className="eyebrow text-[var(--helios-orange)]">Email Studio</p><h2 id="expanded-preview-title" className="mt-1 text-lg font-light text-white sm:text-xl">Expanded live preview</h2></div><button autoFocus type="button" onClick={() => setPreviewExpanded(false)} className="admin-btn-secondary">Close</button></div>{personalizedPreview && <p className="mt-4 text-sm text-white/40">Previewing as {previewProfile.fullName}; final content varies by recipient.</p>}<div className="email-studio-preview-expanded mx-auto max-w-3xl"><EmailTemplatePreview templateKey={templateKey} subject={preview.subject} previewText={preview.previewText} body={preview.body} imageUrl={imageUrl} imageAlt={imageAlt} imageCaption={imageCaption} imageLink={imageLink} /></div></div></div>}
