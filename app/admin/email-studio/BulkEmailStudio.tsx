  ...
  return <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_20rem]">
    ...
    <aside className="space-y-6 xl:self-start">
      <section className="relative rounded-2xl border border-white/[0.08] bg-[#111] p-5">
        <div className="flex items-center justify-between gap-3"><p className="text-xs uppercase tracking-[0.16em] text-white/30">Live preview</p><div className="flex items-center gap-1"><button type="button" onClick={() => setPersonalizedPreview((value) => !value)} className="rounded-md px-2 py-2 text-xs text-[#e7ddc8]/60 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]">{personalizedPreview ? "Personalized" : "Template"}</button><button type="button" onClick={() => setPreviewExpanded(true)} className="rounded-md px-2 py-2 text-xs text-[#e7ddc8]/75 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]" aria-label="Expand live email preview">Expand</button></div></div>
