...
export default function BulkEmailStudio({ clients, groups, campaigns, canSend, defaultTestEmail, initialDraft }: {
  ...
  initialDraft?: { id: string; subject: string; previewText: string | null; body: string; templateKey: string; recipientMode: string; selection: { groupIds?: string[]; clientIds?: string[] } } | null;
}) {
  ...
  async function submit(action: "draft" | "test" | "send" | "schedule") {
    ...
    try {
      ...
      const response = await fetch(rescheduling ? `/api/admin/email-campaigns/${rescheduling.id}` : "/api/admin/email-campaigns", {
        ...
        body: JSON.stringify({
          action: rescheduling ? "reschedule" : action, draftId, subject, previewText, body, templateKey, mode, groupIds, clientIds, testEmail,
      ...
      if (action === "draft") {
        ...
        const saved: Campaign = {
          id: data.campaignId, subject: subject || "Untitled email", previewText, body, templateKey, status: "DRAFT", recipientMode: mode,
      ...
      if (action === "schedule") {
        ...
        if (rescheduling) {
          ...
          id: data.campaignId, subject, previewText, body, templateKey, status: "SCHEDULED", recipientMode: mode,
  ...
  async function manageCampaign(campaign: Campaign, action: "cancel" | "send-now" | "edit") {
    ...
    try {
      ...
      if (action === "edit") {
        setSubject(campaign.subject); setPreviewText(campaign.previewText ?? ""); setBody(campaign.body); setTemplateKey(normalizeEmailTemplateKey(campaign.templateKey));
  ...
  return <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_20rem]">
    ...
    <aside className="space-y-6 xl:self-start">
      <section className="relative rounded-2xl border border-white/[0.08] bg-[#111] p-5">
        ...
        <EmailTemplatePreview templateKey={templateKey} subject={preview.subject} previewText={preview.previewText} body={preview.body} />
      </section>
      <section className="rounded-2xl border border-white/[0.08] bg-[#111] p-5"><p className="text-xs uppercase tracking-[0.16em] text-[var(--helios-orange)]">Campaign history</p><p className="mt-1 text-xs leading-5 text-white/25">Resume drafts or inspect delivered email snapshots.</p><div className="mt-3 divide-y divide-white/[0.06]">{history.map((campaign) => <article key={campaign.id} className="py-3 first:pt-0"><div className="flex items-start justify-between gap-3"><p className="line-clamp-2 text-sm text-white/65">{campaign.subject}</p><span className="text-[10px] uppercase tracking-[0.12em] text-white/30">{campaign.status}</span></div><p className="mt-1 text-xs leading-5 text-white/25">{campaignHistorySummary(campaign)} · {new Date(campaign.createdAt).toLocaleDateString()}</p>{campaign.status === "DRAFT" ? <div className="mt-2 flex flex-wrap items-center gap-4"><button type="button" disabled={busy !== null} onClick={() => resumeDraft(campaign)} className="text-xs font-medium text-[#e7ddc8] disabled:opacity-40">Resume Draft</button><button type="button" disabled={busy !== null} onClick={() => deleteDraft(campaign)} className="text-xs text-red-200/55 transition hover:text-red-200 disabled:opacity-40">{busy === `delete:${campaign.id}` ? "Deleting…" : "Delete Draft"}</button></div> : <button type="button" ref={campaign.id === viewingCampaign?.id ? historyTriggerRef : undefined} aria-label={`View campaign: ${campaign.subject}`} onClick={(event)=>{historyTriggerRef.current=event.currentTarget;setViewingCampaign(campaign);}} className="mt-2 text-xs text-[#e7ddc8]/65">View</button>}{campaign.status === "SCHEDULED" && <div className="mt-2 flex flex-wrap gap-2"><button type="button" onClick={() => openSchedule(campaign)} className="text-xs text-[#e7ddc8]/65">Reschedule</button><button type="button" onClick={() => manageCampaign(campaign, "send-now")} className="text-xs text-[#e7ddc8]/65">Send Now</button><button type="button" onClick={() => manageCampaign(campaign, "edit")} className="text-xs text-white/40">Edit</button><button type="button" onClick={() => manageCampaign(campaign, "cancel")} className="text-xs text-red-200/55">Cancel</button></div>}</article>)}{!history.length && <p className="py-6 text-sm text-white/30">No campaigns yet.</p>}</div></section>
    </aside>
