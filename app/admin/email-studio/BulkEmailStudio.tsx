...
export default function BulkEmailStudio({ clients, groups, campaigns, canSend, defaultTestEmail }: {
  ...
  return <div className="grid gap-7 xl:grid-cols-[minmax(0,1.2fr)_minmax(22rem,.8fr)]">
    ...
    {viewingCampaign && <div role="dialog" aria-modal="true" aria-labelledby="sent-email-title" className="fixed inset-0 z-[100] overflow-y-auto bg-black/85 p-4 backdrop-blur-sm sm:p-8"><div className="mx-auto max-w-3xl rounded-2xl border border-white/10 bg-[#111] p-6 sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow text-[var(--helios-orange)]">Immutable sent snapshot</p><h2 id="sent-email-title" className="mt-3 text-2xl font-light text-white">{viewingCampaign.subject}</h2></div><button autoFocus type="button" aria-label="Close sent email" onClick={()=>{setViewingCampaign(null);window.setTimeout(()=>historyTriggerRef.current?.focus(),0);}} className="admin-btn-secondary">Close</button></div><dl className="mt-6 grid gap-4 text-xs text-white/40 sm:grid-cols-2"><div><dt className="uppercase tracking-[.12em] text-white/25">Sender</dt><dd className="mt-1">{viewingCampaign.createdBy.displayName}</dd></div><div><dt className="uppercase tracking-[.12em] text-white/25">Audience</dt><dd className="mt-1">{viewingCampaign.recipientMode.replaceAll("_"," ")} · {viewingCampaign.recipientCount} recipients</dd></div><div><dt className="uppercase tracking-[.12em] text-white/25">Sent</dt><dd className="mt-1">{viewingCampaign.sentAt ? new Date(viewingCampaign.sentAt).toLocaleString() : "Not recorded"}</dd></div><div><dt className="uppercase tracking-[.12em] text-white/25">Delivery</dt><dd className="mt-1">{viewingCampaign.sentCount} sent · {viewingCampaign.failedCount} failed</dd></div></dl>{viewingCampaign.previewText && <p className="mt-6 text-sm text-white/35">{viewingCampaign.previewText}</p>}<div className="mt-5 whitespace-pre-wrap rounded-xl border border-white/[0.08] bg-black/30 p-5 text-sm leading-7 text-white/65">{viewingCampaign.body}</div><p className="mt-4 text-xs leading-5 text-white/25">Displayed as safe text. Tracking pixels, remote images, and embedded content are not loaded.</p></div></div>}
  </div>;
}
