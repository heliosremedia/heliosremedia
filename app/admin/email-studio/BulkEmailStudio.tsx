...
export default function BulkEmailStudio({ clients, groups, campaigns, canSend, defaultTestEmail, initialDraft }: {
  ...
  return <div className="grid gap-7 xl:grid-cols-[minmax(0,1fr)_20rem]">
    ...
    <aside className="space-y-6 xl:self-start">
      ...
      <section className="relative z-10 rounded-2xl border border-white/[0.08] bg-[#111] p-5"><p className="text-[0.6rem] uppercase tracking-[0.16em] text-[var(--helios-orange)]">Campaign history</p><p className="mt-1 text-[0.58rem] leading-4 text-white/25">Resume drafts or inspect delivered email snapshots.</p><div className="mt-3 divide-y divide-white/[0.06]">{history.map((campaign) => <article key={campaign.id} className="py-3 first:pt-0"><div className="flex items-start justify-between gap-3"><p className="line-clamp-2 text-[0.66rem] leading-4 text-white/65">{campaign.subject}</p><span className="shrink-0 text-[7px] uppercase tracking-[0.12em] text-white/30">{campaign.status}</span></div><p className="mt-1 text-[0.56rem] leading-4 text-white/25">{campaignHistorySummary(campaign)} · {new Date(campaign.createdAt).toLocaleDateString()}</p>{campaign.status === "DRAFT" ? <div className="relative z-20 mt-2 flex flex-wrap items-center gap-1"><button type="button" disabled={busy !== null} onClick={() => resumeDraft(campaign)} className="rounded-md px-2 py-2 text-[0.46rem] font-medium text-[#e7ddc8]/75 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Resume Draft</button><button type="button" disabled={busy !== null} onClick={() => deleteDraft(campaign)} className="rounded-md px-2 py-2 text-[0.46rem] text-red-200/55 transition hover:bg-red-400/[0.08] hover:text-red-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 disabled:opacity-40">{busy === `delete:${campaign.id}` ? "Deleting…" : "Delete Draft"}</button></div> : <button type="button" ref={campaign.id === viewingCampaign?.id ? historyTriggerRef : undefined} aria-label={`View campaign: ${campaign.subject}`} onClick={(event)=>{historyTriggerRef.current=event.currentTarget;setViewingCampaign(campaign);}} className="mt-2 rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)]">View</button>}{campaign.status === "SCHEDULED" && <div className="relative z-20 mt-2 flex flex-wrap gap-1"><button type="button" disabled={busy !== null} onClick={() => openSchedule(campaign)} className="rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Reschedule</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "send-now")} className="rounded-md px-2 py-2 text-[0.46rem] text-[#e7ddc8]/65 transition hover:bg-white/[0.07] hover:text-[#e7ddc8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Send Now</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "edit")} className="rounded-md px-2 py-2 text-[0.46rem] text-white/40 transition hover:bg-white/[0.07] hover:text-white/75 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--helios-orange)] disabled:opacity-40">Edit</button><button type="button" disabled={busy !== null} onClick={() => manageCampaign(campaign, "cancel")} className="rounded-md px-2 py-2 text-[0.46rem] text-red-200/55 transition hover:bg-red-400/[0.08] hover:text-red-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-300 disabled:opacity-40">Cancel</button></div>}</article>)}{!history.length && <p className="py-6 text-[0.66rem] text-white/30">No campaigns yet.</p>}</div></section>
    </aside>
