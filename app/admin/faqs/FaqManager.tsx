                ...
                {selected.faqs.map((faq, index) => (
                  <article key={faq.id} className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-5">
                    ...
                    <div className="mt-4 flex flex-wrap items-center gap-1 border-t border-white/[0.06] pt-3">
                      ...
                      <button type="button" disabled={busyId !== null} onClick={() => toggleFaq(faq)} className="ml-auto admin-btn-link">{faq.published ? "Unpublish" : "Publish"}</button>
                      <button type="button" onClick={() => setDialog({ type: "faq", faq })} className="admin-btn-link">Edit</button>
                      <button type="button" disabled={busyId !== null} onClick={() => deleteFaq(faq)} className="px-3 py-1 admin-btn-link-destructive hover:text-red-200">Delete</button>
                    </div>
                  </article>
                ))}
      ...
      {dialog?.type === "category" && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/80 p-5 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="category-dialog-title">
          <form action={saveCategory} className="w-full max-w-xl rounded-2xl border border-white/[0.1] bg-[#151515] p-6 shadow-2xl sm:p-8">
            ...
            <div className="mt-7 flex justify-end gap-3"><button type="button" onClick={() => setDialog(null)} className="admin-btn-secondary">Cancel</button><button type="submit" disabled={busyId !== null} className="admin-btn-primary">{busyId ? "Saving…" : "Save category"}</button></div>
          </form>
        </div>
      )}
      ...
      {dialog?.type === "faq" && selected && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center overflow-y-auto bg-black/80 p-5 backdrop-blur-md" role="dialog" aria-modal="true" aria-labelledby="faq-dialog-title">
          <form action={saveFaq} className="my-auto w-full max-w-3xl rounded-2xl border border-white/[0.1] bg-[#151515] p-6 shadow-2xl sm:p-8">
            ...
            <label className="mt-5 block text-[0.55rem] font-semibold uppercase tracking-[0.16em] text-white/35">Answer <span className="font-normal normal-case tracking-normal text-white/20">· safe HTML supported</span><textarea name="answer" required maxLength={5000} rows={8} defaultValue={dialog.faq?.answer} className="mt-2 w-full resize-y rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-sm normal-case leading-7 tracking-normal text-white outline-none focus:border-[var(--helios-orange)]" /></label>
            ...
            <div className="mt-7 flex justify-end gap-3"><button type="button" onClick={() => setDialog(null)} className="admin-btn-secondary">Cancel</button><button type="submit" disabled={busyId !== null} className="admin-btn-primary">{busyId ? "Saving…" : "Save question"}</button></div>
          </form>
        </div>
      )}
    </>
