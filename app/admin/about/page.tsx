...
export default async function AdminAboutPage() {
  ...
  const [content, teamMembers] = await Promise.all([getAboutPageContent(session.workspaceId), prisma.teamMember.findMany({ where: await getContentOwnershipScope(session.workspaceId), orderBy: [{ displayOrder: "asc" }, { name: "asc" }], select: teamMemberSelect })]);
