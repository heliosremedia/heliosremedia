...
export async function GET(request: Request) {
  ...
  try {
    ...
    await recordAuditEvent({ workspaceId: session.workspaceId, actorId: session.userId, actorEmail: session.email, action: "GOOGLE_BUSINESS_AUTHORIZED", entityType: "GoogleBusinessConnection", summary: selected ? "Google Business Profile connected." : "Google authorization completed; location selection is required." });
