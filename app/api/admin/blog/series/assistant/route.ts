...
export async function POST(request: Request) {
  ...
  try {
    ...
    await recordAuditEvent({ workspaceId: session.workspaceId, actorId:session.userId, actorEmail:session.email, action:"BLOG_SERIES_AI_PROPOSAL_REQUESTED", entityType:"BlogSeries", summary:"Requested a temporary Blog Series proposal.", metadata:{ workspaceId:session.workspaceId, regeneration:Boolean(regenerateField) } });
