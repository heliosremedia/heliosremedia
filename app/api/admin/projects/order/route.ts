...
export async function PATCH(request: Request) {
  try {
    const body = (await request.json()) as { projectIds?: unknown; startingDisplayOrder?: unknown };
    const projectIds = Array.isArray(body.projectIds) ? body.projectIds.filter((id): id is string => typeof id === "string" && id.length > 0) : [];
    const startingDisplayOrder = typeof body.startingDisplayOrder === "number" && Number.isSafeInteger(body.startingDisplayOrder) && body.startingDisplayOrder >= 0 ? body.startingDisplayOrder : 0;
    ...
    await prisma.$transaction(projectIds.map((id, index) => prisma.project.update({ where: { id }, data: { displayOrder: startingDisplayOrder + index } })));
