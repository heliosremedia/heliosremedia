...
export async function DELETE(request: Request, { params }: Context) {
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      await requireLockedWorkspaceEditor(tx, session);
      const locked = await tx.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "Project" WHERE "id"=${projectId} AND "workspaceId"=${session.workspaceId} FOR UPDATE`;
      if (!locked.length) return { count: 0 };
      return tx.projectPreviewLink.updateMany({ where: { id: previewId, projectId, project: { workspaceId: session.workspaceId }, revokedAt: null }, data: { revokedAt: new Date() } });
    });
