...
export async function PUT(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const users=await prisma.adminUser.findMany({where:{id:{in:userIds},workspaceId:session.workspaceId,active:true},select:{id:true,displayName:true,disciplines:true}});
  ...
  const rows=body.contributors.map((item,index)=>{
    ...
    return {projectId,workspaceId:session.workspaceId,adminUserId:user?.id||null,displayNameSnapshot:user?.displayName||externalName,disciplinesSnapshot:user?.disciplines||[externalDiscipline||"Other"],externalName:user?null:externalName,externalDiscipline:user?null:externalDiscipline||null,public:item.public!==false,displayOrder:index};
  });
  try{
