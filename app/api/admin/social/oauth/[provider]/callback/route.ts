...
export async function GET(request:Request,{params}:{params:Promise<{provider:string}>}) {
  ...
  const separator=saved.lastIndexOf("."); const expected=saved.slice(0,separator); const userId=saved.slice(separator+1);
  if(!code || userId!==session.userId || !verifyOAuthState(expected,state)) return NextResponse.redirect(new URL("?connection=invalid-state",settingsUrl));
