...
export async function POST(request:Request){
 ...
 const workspaceId=await requireWorkspaceId(session.userId);const connectionId=clean(body.connectionId);const connection=await prisma.socialConnection.findFirst({where:{id:connectionId,workspaceId},select:{id:true,state:true}});
