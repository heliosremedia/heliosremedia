...
export async function POST(request: Request) {
  ...
  const invitation = await prisma.adminInvitation.create({ data: { email, displayName, role, tokenHash: hashInvitationToken(token), createdById: session.userId, workspaceId: session.workspaceId, expiresAt: new Date(Date.now() + 7 * 86400000) } });
...
export async function PATCH(request: Request) {
  ...
  const updated = await prisma.adminUser.update({ where: { id: userId }, data: { ...(role ? { role } : {}), ...(active !== null ? { active, state: active ? "ACTIVE" : "DEACTIVATED" } : {}), ...(nextDisplayName !== null ? { displayName: nextDisplayName } : {}), ...(firstName !== undefined ? { firstName } : {}), ...(lastName !== undefined ? { lastName } : {}), ...(phone !== undefined ? { phone } : {}), ...(selectedDisciplines ? { disciplines: selectedDisciplines } : {}), ...(passwordHash ? { passwordHash, failedLoginCount: 0, lockedUntil: null } : {}), ...(revokeSessions ? { sessionVersion: { increment: 1 } } : {}) }, select: { id: true, displayName: true, firstName: true, lastName: true, phone: true, disciplines: true, role: true, active: true, state: true } });
