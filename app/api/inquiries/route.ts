...
import { canUseLegacyInquiryNotifications } from "@/lib/inquiry-ownership";
...
export async function POST(request: Request) {
  try {
    ...
    if (!await canUseLegacyInquiryNotifications(workspaceId)) return NextResponse.json({ success: false, error: "Inquiry delivery is not configured for this company." }, { status: 503 });
    const hash = ipHash(request);
    const recent = await prisma.inquiry.count({ where: { ...await getContentOwnershipScope(workspaceId), ipHash: hash, createdAt: { gte: new Date(Date.now() - 60 * 60_000) } } });
