...
export async function POST(request: Request) {
  try {
    ...
    const inquiry = await prisma.inquiry.create({ data: { name: text(body.name, 120, true)!, email: email(body.email), phone: text(body.phone, 40), propertyAddress: text(body.propertyAddress, 240), city: text(body.city, 120), state: text(body.state, 40), postalCode: text(body.postalCode, 20), desiredDate, message: text(body.message, 3000), sourcePage: text(body.sourcePage, 1000), referrer: text(body.referrer, 1000), ctaName: text(body.ctaName, 120), utmSource: text(body.utmSource, 240), utmMedium: text(body.utmMedium, 240), utmCampaign: text(body.utmCampaign, 240), utmContent: text(body.utmContent, 240), utmTerm: text(body.utmTerm, 240), ipHash: hash, userAgent: text(request.headers.get("user-agent"), 500), consent: true, requestedServices: { create: validServices.map(({ id }) => ({ serviceId: id })) }, activities: { create: { action: "INQUIRY_CREATED", summary: "Inquiry submitted through the public website." } } }, select: { id: true } });
