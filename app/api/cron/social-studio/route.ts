...
import { checkMetaConnectionHealth } from "@/lib/social/meta-health";
...
export async function GET(request: Request) {
  ...
  const [manual, publishing, analytics, metaHealth] = await Promise.all([processDueSocialVariants(), processPublishingQueue(), processAnalyticsQueue(),checkMetaConnectionHealth()]);
  return NextResponse.json({ success: true, manual, publishing, analytics,metaHealth });
}
