...
export default async function PhotographyGalleryPage() {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-white"><Navbar variant="solid"/><PortfolioAnalytics page="portfolio"/><section className="container-shell pb-24 pt-28 sm:pb-32 sm:pt-32"><Link href="/portfolio" className="text-[0.56rem] font-semibold uppercase tracking-[0.16em] text-white/40 hover:text-white">← Portfolio</Link><div className="mt-7 border-b border-white/[0.08] pb-6"><p className="eyebrow text-[var(--helios-orange)]">Quick Browse</p><h1 className="mt-4 max-w-4xl font-display text-[clamp(3rem,6vw,5.5rem)] font-light leading-[0.94] tracking-[-0.045em]">All Photography</h1><p className="mt-4 max-w-2xl text-sm leading-6 text-white/42">A broad look at published Helios photography, arranged as a balanced editorial collection.</p><p className="mt-2 text-xs text-white/28">{result.total} photographs</p></div><PhotographyBrowser initialItems={result.items} initialNextOffset={result.nextOffset} total={result.total}/></section><Footer/></main>;
}
