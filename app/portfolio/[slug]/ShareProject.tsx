...
export default function ShareProject({ url, title, summary }: { url: string; title: string; summary: string }) {
  ...
  return <div ref={rootRef} className="relative inline-flex">
    <button ref={triggerRef} type="button" onClick={activate} aria-haspopup="menu" aria-expanded={open} className="public-btn public-btn-outline public-btn-compact"><svg aria-hidden="true" viewBox="0 0 24 24" className="h-4 w-4 fill-none stroke-current"><circle cx="18" cy="5" r="2.5"/><circle cx="6" cy="12" r="2.5"/><circle cx="18" cy="19" r="2.5"/><path d="m8.2 10.8 7.5-4.4M8.2 13.2l7.5 4.4"/></svg>{copied?"Link copied":"Share project"}</button>
