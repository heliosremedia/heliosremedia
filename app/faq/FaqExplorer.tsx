          ...
          <div className="lg:sticky lg:top-8">
            ...
            <div className="mt-9" role="navigation" aria-label="FAQ categories">
              ...
              <div className="mt-4 flex flex-wrap gap-2 lg:flex-col">
                <button type="button" onClick={() => setSelectedSlug("all")} aria-pressed={selectedSlug === "all"} className={`rounded-full border px-4 py-2.5 text-left text-[0.58rem] font-semibold uppercase tracking-[0.15em] transition ${selectedSlug === "all" ? "border-[var(--helios-orange)]/45 bg-[var(--helios-orange)]/[0.09] text-[var(--helios-orange)]" : "border-white/[0.09] text-white/35 hover:border-white/20 hover:text-white"}`}>All questions</button>
                {categories.map((category) => <button key={category.id} type="button" onClick={() => setSelectedSlug(category.slug)} aria-pressed={selectedSlug === category.slug} className={`rounded-full border px-4 py-2.5 text-left text-[0.58rem] font-semibold uppercase tracking-[0.15em] transition ${selectedSlug === category.slug ? "border-[var(--helios-orange)]/45 bg-[var(--helios-orange)]/[0.09] text-[var(--helios-orange)]" : "border-white/[0.09] text-white/35 hover:border-white/20 hover:text-white"}`}>{category.name}</button>)}
              </div>
            </div>
          </div>
        </aside>
