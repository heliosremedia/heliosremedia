        ...
        <aside>
          <div className="lg:sticky lg:top-8">
            ...
            <div className="mt-9" role="navigation" aria-label="FAQ categories">
              ...
              <div className="mt-3 flex max-w-full gap-1.5 overflow-x-auto pb-2 [scrollbar-width:none] lg:flex-col lg:overflow-visible lg:pb-0 [&::-webkit-scrollbar]:hidden">
                <button type="button" onClick={() => setSelectedSlug("all")} aria-pressed={selectedSlug === "all"} className={`min-h-11 shrink-0 rounded-full border px-4 py-2 text-left text-[0.66rem] font-semibold uppercase tracking-[0.08em] transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--helios-orange)] ${selectedSlug === "all" ? "border-[var(--helios-orange)]/45 bg-[var(--helios-orange)]/[0.09] text-[var(--helios-orange)]" : "border-white/[0.09] text-white/35 hover:border-white/20 hover:text-white"}`}>All questions</button>
                {categories.map((category) => <button key={category.id} type="button" onClick={() => setSelectedSlug(category.slug)} aria-pressed={selectedSlug === category.slug} className={`min-h-11 shrink-0 rounded-full border px-4 py-2 text-left text-[0.66rem] font-semibold uppercase tracking-[0.08em] transition focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--helios-orange)] ${selectedSlug === category.slug ? "border-[var(--helios-orange)]/45 bg-[var(--helios-orange)]/[0.09] text-[var(--helios-orange)]" : "border-white/[0.09] text-white/35 hover:border-white/20 hover:text-white"}`}>{category.name}</button>)}
              </div>
            </div>
          </div>
        </aside>
